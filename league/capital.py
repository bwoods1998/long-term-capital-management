...
def kelly_stake(growth: list[float], present_stake: Decimal, venue_cash: Decimal, *, alpha: float | None = None) -> tuple[Decimal, dict[str, Any]]:
    ...
    fraction = stats.quarter_kelly(bounds["lcb"], bounds["sd"] ** 2, fraction=float(rules["kelly_fraction"]), cap=1e9)
    numbers.update(mean=bounds["mean"], lcb=bounds["lcb"], variance=bounds["sd"] ** 2, scale=fraction)
    if fraction <= 0:
    ...
    return max(floor, min(target, ceiling)), {**numbers, "reason": "a quarter of Kelly on the lower bound"}
