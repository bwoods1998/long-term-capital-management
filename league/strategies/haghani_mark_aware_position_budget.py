...
# Preserve the maker-reversion signal and exits; bound entry quantities
# by current market value as well as the resting limit price.
...
def decide(ctx):
    ...
    position_budget = max(0.0, _num(limits.get("max_position_usd"))) * 0.98
    ...
    for symbol in NEEDS["symbols"]:
        ...
        valuation = max(target, bid, _num((quotes.get(symbol) or {}).get("ask")), _num(rows[-1].get("c")))
        if buys:
            committed = sum(max(0.0, _num(o.get("quantity")) - _num(o.get("filled"))) * max(valuation, _num(o.get("limit_price"))) for o in buys)
            if position_budget <= 0 or committed > position_budget:
                _cancel(cancels, buys)
                notes.append(f"{symbol}: cancelling dip bids valued at ${committed:.2f}, above the buffered position budget ${position_budget:.2f}")
            else:
        ...
        # Use explicit quantity: a discounted limit must not turn a dollar
        # budget into a larger position when valued at the current market.
        budget = max(0.0, min(want, _num(limits.get("max_order_usd"), want), position_budget, free))
        quantity = math.floor(budget / valuation * 1_000_000_000) / 1_000_000_000 if valuation > 0 else 0.0
        dollars = quantity * target
        minimum = max(10.0, _num(((ctx.get("venue_rules") or {}).get(symbol) or {}).get("min_order_usd")))
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or quantity <= 0 or dollars < minimum:
            notes.append(f"{symbol}: no bid placed (dip price {target:.2f}, bid {bid:.2f}, ${max(dollars, 0.0):.2f} spendable)")
            continue
        free -= quantity * valuation
        intents.append({"symbol": symbol, "side": "buy", "quantity": quantity, "type": "limit", "limit_price": target,
                        "reason": (f"Resting a limit buy of {quantity:.9f} {symbol} (${dollars:.2f} at the limit), "
                                   f"with current-value exposure bounded by the buffered position budget: "
                                   f"{(1.0 - target / mean) * 100.0:.1f}% under the {window}-bar mean {mean:.2f} "
                                   f"({k:.1f} x the average range {atr:.2f}). It fills only on a sharp dip, as a maker; exit at the mean.")})
