# Corrected child of haghani-38, family crypto-alts-reversion.
# Preserve the specialty and trading logic; use venue grids rather than cents.
...
def _grid_price(price, increment, side):
    """Use only a stated positive grid; never make an order more aggressive."""
    step = _num(increment)
    if step <= 0:
        return price
...
def decide(ctx):
    ...
    for symbol in NEEDS["symbols"]:
        ...
        exit_target = None if mean is None else _grid_price(mean, increment, "sell")
        ...
        if position is not None:
            ...
            if why:
                ...
                notes.append(f"{symbol}: holding, the sell at the mean {'is being moved' if stale else 'rests'} near {exit_target:.8g}")
            else:
                intents.append({"symbol": symbol, "side": "sell", "quantity": position["quantity"], "type": "limit", "limit_price": exit_target,
                                "reason": (f"Resting a limit sell of all {symbol} at {exit_target:.8g}, the venue-grid exit at or above the mean of the last {window} 15-minute closes: "
                                           f"bought at {cost:.2f}, the trade is over when the price is back at its mean.")})
                notes.append(f"{symbol}: holding, resting the exit at {exit_target:.8g}")
            continue
        ...
        if mean is None:
            ...
            continue
        target = _grid_price(min(mean - k * atr, mean * (1.0 - edge_pct / 100.0)), increment, "buy")
        if buys:
            stale = [o for o in buys if _moved(o, target, replace_pct)] + buys[1:]
            _cancel(cancels, stale)
            notes.append(f"{symbol}: the dip bid {'is being moved' if stale else 'rests'} near {target:.8g}")
            continue
