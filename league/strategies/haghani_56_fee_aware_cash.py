...
def decide(ctx):
    ...
    fee_rate = max(0.0, num(fees.get('crypto_taker'), 0.0025),
                   num(fees.get('crypto_maker'), 0.0015))
    cash_left = max(0.0, num(ctx.get('cash')))
    # Keep commitments reserved even if this decision requests cancellation.
    # Do not count sale proceeds until a later snapshot confirms them.
    for order in orders:
        if order.get('side') != 'buy':
            continue
        remaining = max(0.0, num(order.get('quantity')) - num(order.get('filled')))
        if remaining <= 0:
            continue
        ask = num((quotes.get(order.get('symbol')) or {}).get('ask'))
        price = max(num(order.get('limit_price')), ask)
        if price <= 0:
            cash_left = 0.0
            break
        cash_left -= remaining * price * (1.0 + fee_rate) + 0.01
    cash_left = max(0.0, cash_left)
    for symbol in NEEDS['symbols']:
        ...
        if dollars < min_order:
            notes.append(symbol + ': cash or limits below minimum order after fees and commitments')
            continue
