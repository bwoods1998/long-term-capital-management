...
PARAMS = {'bid_min': 0.9, 'bid_max': 0.97, 'max_spread': 0.03, 'min_hours': 1.0, 'max_hours': 30.0, 'min_volume_24h': 1000, 'notional_usd': 10.0, 'max_open': 6, 'requote_minutes': 180}
LEG = 'no'
EPS = 1e-9
...
def decide(ctx):
    ...
    for minus_volume, ticker, bid, ask, hours in found:
        ...
        used[event] = used.get(event, 0.0) + cost
        busy.add(ticker)
        intents.append({'market': ticker, 'leg': LEG, 'side': 'buy', 'quantity': quantity, 'type': 'limit', 'limit_price': bid, 'post_only': True, 'reason': f'Resting post-only NO bid at {bid:.2f}; favourite in target band, {hours:.1f}h to close, sized within remaining event allowance.'})
    return {'intents': intents, 'cancels': cancels, 'thought': f'Saw {len(markets)} markets and {len(found)} eligible favourites. Rested {len(intents)} event-budgeted bids and cancelled {len(cancels)} stale orders.', 'memory': {}}
