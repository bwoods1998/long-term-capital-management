# Corrected child of haghani; family: crypto-alts-reversion.
# Same markets, signal, sizing and exits; fresh rung-0 evidence required.
# Retain cent precision, but never round a dip-buy price above its ceiling.
# Limit orders are not proof of maker execution; inspect actual fills and fees.
...
def _entry_price(ceiling):
    """Keep cent precision without ever increasing the entry ceiling."""
    if not math.isfinite(ceiling) or ceiling <= 0:
        return 0.0
    cents = (Decimal(str(ceiling)) * 100).to_integral_value(rounding=ROUND_FLOOR)
    target = float(cents / 100)
    return target if 0 < target <= ceiling else 0.0
    ...
    for symbol in NEEDS['symbols']:
        ...
        if position is not None:
            ...
            if why:
                ...
                state = 'is being moved' if stale else 'rests'
                notes.append(f'{symbol}: holding, the sell at the mean {state} near {mean:.2f}')
            else:
        ...
        if mean is None:
            ...
            continue
        ceiling = min(mean - k * atr, mean * (1.0 - edge_pct / 100.0))
        target = _entry_price(ceiling)
        if buys:
            # The entry ceiling is mandatory, not subject to replacement tolerance.
            stale = [o for o in buys if _num(o.get('limit_price')) > ceiling or _moved(o, target, replace_pct)] + buys[1:]
            _cancel(cancels, stale)
            state = 'is being moved' if stale else 'rests'
            notes.append(f'{symbol}: the dip bid {state} near {target:.8f}')
            continue
        ...
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < 1.0:
            notes.append(f'{symbol}: no bid placed (dip price {target:.8f}, bid {bid:.8f}, ${max(dollars, 0.0):.2f} spendable)')
            continue
        free -= dollars
        intents.append({'symbol': symbol, 'side': 'buy', 'notional_usd': dollars, 'type': 'limit', 'limit_price': target,
                        'reason': (f'Limit buy of ${dollars:.2f} of {symbol} at {target:.8f}: {(1.0 - target / mean) * 100.0:.4f}% under the '
                                   f'{window}-bar mean {mean:.8f}. Rounded downward from the entry ceiling {ceiling:.8f} '
                                   f'({k:.1f} x average range {atr:.8f}, minimum discount {edge_pct:.4f}%). '
                                   'Below the observed bid; maker execution is not guaranteed. Exit at the mean.')})
        notes.append(f'{symbol}: placing a dip bid at {target:.8f}')
