...
def decide(ctx):
    ...
    # A date rollover makes an exit due, not an overnight market order valid.
    if now.weekday() >= 5 or not 570 <= minute < 960:
        return {'intents': [], 'cancels': [],
                'thought': 'Outside weekday regular-session hours; defer market orders and keep any overnight exit due.',
                'memory': memory}
