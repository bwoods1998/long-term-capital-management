...
# Preserve the maker-reversion signals and exits; cap entries by current equity.
...
def decide(ctx):
    ...
    entry_cap = max(0.0, min(0.5 * _num(ctx.get("equity")),
                             _num(limits.get("max_order_usd"), want),
                             _num(limits.get("max_position_usd"), want)))
    ...
    for symbol in NEEDS["symbols"]:
        ...
        if buys:
            oversized = [o for o in buys
                         if max(0.0, _num(o.get("quantity")) - _num(o.get("filled")))
                         * _num(o.get("limit_price")) > entry_cap]
            stale = [o for o in buys if _moved(o, target, replace_pct)] + buys[1:] + oversized
            _cancel(cancels, stale)
            notes.append(f"{symbol}: dip bid {'cancelled for size or repricing' if stale else 'rests'} near {target:.2f}")
            continue
        age = _hours_between(rows[-1].get("t"), ctx.get("now"))
        dollars = math.floor(min(want, entry_cap, free) * 100.0) / 100.0
        rules = (ctx.get("venue_rules") or {}).get(symbol) or {}
        minimum = max(10.0, _num(rules.get("min_order_usd"), 10.0))
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < minimum:
