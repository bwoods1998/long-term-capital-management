...
# Preserve the parent's specialty and decision logic except for the buy minimum.
# Skip undersized entries rather than increasing the capped dollar budget.
...
def decide(ctx):
    ...
    for symbol in NEEDS["symbols"]:
        ...
        minimum = max(10.0, _num(((ctx.get("venue_rules") or {}).get(symbol) or {}).get("min_order_usd"), 10.0))
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < minimum:
            notes.append(f"{symbol}: no bid placed (dip price {target:.2f}, bid {bid:.2f}, ${max(dollars, 0.0):.2f} spendable, ${minimum:.2f} entry minimum)")
            continue
        free -= dollars
