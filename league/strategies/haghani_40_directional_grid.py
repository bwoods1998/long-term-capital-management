# Corrected child of haghani-40; family: crypto-alts-reversion.
# Preserve the parent's dip-entry / mean-exit policy without assuming cent grids.
import math
...
def _limit_price(price, increment, side):
    """Use only a stated venue grid, never making the price more aggressive."""
    step = _num(increment)
    if step <= 0:
        return price
    grid = Decimal(str(step))
    rounding = ROUND_FLOOR if side == 'buy' else ROUND_CEILING
    units = (Decimal(str(price)) / grid).to_integral_value(rounding=rounding)
...
def _mean_and_atr(rows, window):
    """Mean close and average true range; needs one preceding bar."""
...
def decide(ctx):
    ...
    window, k, replace_pct = max(2, int(_num(p.get('window'), 33))), _num(p.get('k'), 2.0), _num(p.get('replace_pct'), 0.2)
    ...
    venue_rules = ctx.get('venue_rules') or {}
    ...
    for symbol in NEEDS['symbols']:
        ...
        increment = (venue_rules.get(symbol) or {}).get('price_increment')
        exit_price = None if mean is None else _limit_price(mean, increment, 'sell')
        ...
        if position is not None:
            ...
            if loss_pct >= stop_pct:
                why = f'stop: the bid {price:.10g} is {loss_pct:.1f}% under the average cost {cost:.10g} (limit {stop_pct:.1f}%)'
            ...
            if why:
                ...
                stale = [o for o in sells if _moved(o, exit_price, replace_pct)] + sells[1:]
                _cancel(cancels, stale)
                notes.append(f"{symbol}: holding, the sell at the mean {'is being moved' if stale else 'rests'} near {exit_price:.10g}")
            else:
                intents.append({'symbol': symbol, 'side': 'sell', 'quantity': position['quantity'], 'type': 'limit', 'limit_price': exit_price,
                                'reason': (f'Resting a limit sell of all {symbol} at {exit_price:.10g}, at or above the mean {mean:.10g} of the last {window} 15-minute closes: '
                                           f'bought at {cost:.10g}, the trade is over when the price is back at its mean.')})
                notes.append(f'{symbol}: holding, resting the exit at {exit_price:.10g}')
            continue
        ...
        if mean is None:
            ...
            continue
        target = _limit_price(min(mean - k * atr, mean * (1.0 - edge_pct / 100.0)), increment, 'buy')
        if buys:
            stale = [o for o in buys if _moved(o, target, replace_pct)] + buys[1:]
            _cancel(cancels, stale)
            notes.append(f"{symbol}: the dip bid {'is being moved' if stale else 'rests'} near {target:.10g}")
            continue
        ...
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < 1.0:
            notes.append(f'{symbol}: no bid placed (dip price {target:.10g}, bid {bid:.10g}, ${max(dollars, 0.0):.2f} spendable)')
            continue
        free -= dollars
        intents.append({'symbol': symbol, 'side': 'buy', 'notional_usd': dollars, 'type': 'limit', 'limit_price': target,
                        'reason': (f'Resting a limit buy of ${dollars:.2f} of {symbol} at {target:.10g}: {(1.0 - target / mean) * 100.0:.1f}% under the '
                                   f'{window}-bar mean {mean:.10g} ({k:.1f} x the average range {atr:.10g}). It fills only on a sharp dip, as a maker; exit at the mean.')})
        notes.append(f'{symbol}: resting a dip bid at {target:.10g}')
