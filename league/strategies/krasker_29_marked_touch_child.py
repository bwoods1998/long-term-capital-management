# Corrected child of krasker-29; family: options-orb-vertical.
# Preserve the parent's opening-range fade and exit triggers, but close at
# the marked touch rather than offering half the mark or a penny for urgency.
...
def _vertical(ctx, under, bullish, kind, p, ny, budget):
    # Match the parent's delta, expiry and width selection.
...
def _exits(ctx, ny, p, notes, signal_exit):
    # Preserve stale-order management and all target, stop, signal and time triggers.
    ...
    for row in [r for r in ctx.get("positions") or [] if isinstance(r, dict) and len(_occs(r)) >= 2 and _num(r.get("quantity")) >= 1]:
        ...
        room = width - paid if kind in ("debit_vertical", "credit_vertical") else paid
        unit = width - paid if credit else paid
        ...
        if tuple(occs) in resting or not why:
            ...
            continue
        # The book marks structures at the bid in S units. Do not halve that
        # reference or replace it with a penny when an exit becomes urgent.
        natural = round(max(0.01, mark), 2)
        natural = round(max(0.01, min(width - 0.01, width - natural)), 2) if credit else natural
...
def _enter(ctx, p, ny, notes, cancels, memory, signal, build):
    # An event is spent once a structure sent for it is seen held.
...
NEEDS = {'venue': 'alpaca',
 ...
 'asset_class': 'option',
 'structures': True,
 'symbols': ['IWM', 'BAC', 'SOFI', 'SNAP', 'AAL'],
 'bars': {'timeframe': '15Min', 'limit': 60},
 'max_days_to_expiry': 4,
 'wake_minutes': 5,
 'parameter_rules': {'bounds': {'width': [1, 20],
                                'dte_min': [0, 4],
                                'dte_max': [0, 4],
                                'wing_delta': [0.02, 0.3],
                                'entry_delta': [0.2, 0.6],
                                'profit_target': [0.2, 0.95],
                                'stop_loss': [0.2, 1.0],
                                'exit_minutes_before_close': [15, 240],
                                'exit_dte': [0, 99],
                                'max_open': [1, 3],
                                'max_qty': [1, 3],
                                'notional_usd': [20, 75],
...
PARAMS = {'structure': 'debit_vertical',
 'width': 1.0,
 'dte_min': 0,
 'dte_max': 4,
 'wing_delta': 0.25,
...
def _range(ctx, under, ny, n):
    # Today's closed 15-minute bars: opening range, day open and last close.
...
def decide(ctx):
    ...
    def failed(row, held_kind, occs, dte):
