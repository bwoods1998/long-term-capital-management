# Corrected child of haghani-r42c38c; family crypto-alts-reversion.
# Preserve the cash repair; bound dip quantities at the ask below current equity.
import math
...
def decide(ctx):
    ...
    fee = max(0.0, num(fees.get('crypto_maker'), 0.0015),
              num(fees.get('crypto_taker'), 0.0025))
    cash_left = max(0.0, num(ctx.get('cash')) - 0.01)
    # Keep outstanding buys reserved even when requesting their cancellation.
    # Never spend anticipated proceeds from a sell in this decision.
    for order in orders:
        ...
        if remaining <= 0:
            continue
        ask = num((quotes.get(order.get('symbol')) or {}).get('ask'))
        reserve_price = max(num(order.get('limit_price')), ask)
        if reserve_price <= 0:
            cash_left = 0.0
            continue
        cash_left = max(0.0, cash_left - remaining * reserve_price * (1.0 + fee))
    for symbol in NEEDS['symbols']:
        ...
        for o in sells:
            if o['order_id'] not in cancels: cancels.append(o['order_id'])
        if bid < min_price or bid <= 0 or ask <= 0 or ask < bid:
            for o in buys:
                if o['order_id'] not in cancels: cancels.append(o['order_id'])
        ...
        committed = sum(max(0.0, num(o.get('quantity')) - max(0.0, num(o.get('filled'))))
                        * max(ask, num(o.get('limit_price'))) for o in buys)
        if committed > position_budget or any(num(o.get('quantity'), -1.0) < 0 for o in buys):
            for o in buys:
                if o['order_id'] not in cancels: cancels.append(o['order_id'])
            notes.append(symbol + ': cancelling bids outside the current ask-valued position budget')
            continue
        ...
        if bid <= 0 or target <= 0 or target >= bid:
            continue
        max_order = max(0.0, num(limits.get('max_order_usd')))
        budget = max(0.0, min(notional, max_order, position_budget, cash_left / (1.0 + fee)))
        # Explicit quantity removes any dependence on notional conversion price.
        # Value the whole new position at the ask, not the lower resting bid.
        quantity = float((Decimal(str(budget)) / Decimal(str(ask))).quantize(
        ...
        if quantity <= 0 or dollars < min_order:
            notes.append(symbol + ': cash or ask-valued limits below minimum order')
            continue
        intents.append({'symbol': symbol, 'side': 'buy', 'quantity': quantity,
                        'type': 'limit', 'limit_price': target, 'post_only': True,
                        'reason': 'Rest a post-only maker bid ' + str(entry*100.0) + '% below the current bid for a deep-dip reversion entry, with quantity valued at the ask below current equity and position limits.'})
        cash_left = max(0.0, cash_left - quantity * ask * (1.0 + fee))
        notes.append(symbol + ': resting ask-budgeted deep-dip maker bid')
