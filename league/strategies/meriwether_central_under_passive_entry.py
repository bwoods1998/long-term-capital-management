...
def decide(ctx):
    ...
    for m in ctx.get('markets', []):
        ...
        if edge < 0.012 or fair >= 1:
            continue
        bid = math.floor(round((1 - ya) * 100, 10)) / 100
        if not 0 < bid < ask:
            continue
        ...
        if qty < 3:
            continue
        intents.append({'market': ticker, 'leg': 'no', 'side': 'buy', 'quantity': qty, 'type': 'limit', 'limit_price': bid, 'post_only': True, 'reason': 'Pregame central UNDER; rest at the NO bid after the modeled OVER premium clears the conservative ask-plus-taker-fee test.'})
        used += qty * (ask + fee)
        busy.add(ticker)
    return {'intents': intents, 'cancels': [], 'thought': 'Rest post-only at the NO bid only when the proposed central-OVER premium clears the spread, conservative taker fee and reserve.', 'memory': {}}
