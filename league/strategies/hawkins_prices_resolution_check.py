...
def within_resolution_horizon(row):
    hours = num(row.get('hours_to_resolve'))
...
def decide(ctx):
    ...
    for order in orders:
        ...
        if row is not None and q is not None:
            ...
            invalid = (
                not within_resolution_horizon(row) or
    ...
    for row in markets:
        ...
        hours = num(row.get('hours_to_close'))
        volume = num(row.get('volume_24h'), 0.0)
        if q is None or hours is None or not within_resolution_horizon(row):
            continue
        bid, ask = q
    ...
    for unused, market, bid, ask, hours in candidates:
        ...
        intents.append({
            ...
            'limit_price': bid,
            'post_only': True,
            'reason': 'Post-only YES bid on a liquid WTI/diesel favourite within the tested 0.90-0.99 band, with tight spread and expected resolution within 48 hours.'
        })

    return {
        'intents': intents,
        'cancels': cancels,
        'thought': 'Rechecked existing quotes and resolution eligibility, cancelled stale or invalid rests, and ranked eligible favourites by volume.',
        'memory': {}
    }
