# Corrected child of haghani-45, family crypto-alts-reversion.
# Same specialty and decision rules; price limits on the known venue grid.
# This child starts on rung 0 with no inherited trading evidence.
...
def _limit_price(ctx, symbol, price, side):
    """Never make a limit more aggressive; do not invent an unknown grid."""
    rules = (ctx.get("venue_rules") or {}).get(symbol) or {}
    increment = _num(rules.get("price_increment"))
    if increment <= 0:
        return price
...
def decide(ctx):
    ...
    for symbol in NEEDS["symbols"]:
        ...
        exit_price = _limit_price(ctx, symbol, mean, "sell") if mean is not None else None
        ...
        if position is not None:
            ...
            if why:
                ...
                notes.append(f"{symbol}: holding, the sell at the mean {'is being moved' if stale else 'rests'} near {exit_price}")
            else:
                intents.append({"symbol": symbol, "side": "sell", "quantity": position["quantity"], "type": "limit", "limit_price": exit_price,
                                "reason": (f"Resting a limit sell of all {symbol} at {exit_price}, the mean of the last {window} 15-minute closes rounded up only to a known venue grid: "
                                           f"bought at {cost:.2f}, the trade is over when the price is back at its mean.")})
                notes.append(f"{symbol}: holding, resting the exit at {exit_price}")
            continue
        ...
        if mean is None:
            ...
            continue
        target = _limit_price(ctx, symbol, min(mean - k * atr, mean * (1.0 - edge_pct / 100.0)), "buy")
        if buys:
        ...
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < 10.0:
