...
def payout_eligible(row):
    if not isinstance(row, dict):
        return False
    hours = num(row.get('hours_to_resolve'))
    # Trading close is not payout time. Unknown timing cannot authorize entry.
...
def decide(ctx):
    ...
    for order in orders:
        ...
        if aged or gone or invalid or not payout_eligible(row):
            cancels.append(str(order['order_id']))

    candidates = []
    for row in markets:
        if not payout_eligible(row):
            continue
        q = touch(row)
        hours = num(row.get('hours_to_close'))
        volume = num(row.get('volume_24h'), 0.0)
    ...
    return {
        'intents': intents,
        'cancels': cancels[:20],
        'thought': 'Rechecked quotes and payout timing, cancelled stale or invalid rests, and ranked eligible favourites by volume.',
        'memory': {}
    }
