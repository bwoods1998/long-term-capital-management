...
def decide(ctx):
    ...
    for q in chain:
        if q.get("occ"):
            bysym[str(q["occ"])]=q
        if q.get("symbol") and q.get("symbol")!=q.get("underlying") and q.get("symbol") not in NEEDS["symbols"]:
            bysym[str(q["symbol"])]=q
    for h in ctx.get("positions") or []:
        if not isinstance(h,dict):continue
        sym=str(h.get("occ") or h.get("symbol") or ""); q=bysym.get(sym)
        if not q or not fresh(stamp(q),utc,p["freshness_minutes"]):continue
        sym=str(q.get("occ") or q.get("symbol") or "")
    ...
    for s in NEEDS["symbols"]:
        ...
        ask=num(q.get("ask")); sym=str(q.get("occ") or q.get("symbol") or "")
