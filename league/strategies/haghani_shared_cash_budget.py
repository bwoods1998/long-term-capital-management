...
def decide(ctx):
    ...
    for symbol in NEEDS['symbols']:
        ...
        dollars = min(notional, max_order, max_position, cash_left / (1.0 + fee))
        min_order = num((rules.get(symbol) or {}).get('min_order_usd'), 10.0)
        dollars = math.floor(dollars*100.0)/100.0
        if dollars < min_order:
            notes.append(symbol + ': cash or limits below minimum order')
            continue
        ...
        cash_left = max(0.0, cash_left - dollars * (1.0 + fee))
        notes.append(symbol + ': resting deep-dip maker bid')
