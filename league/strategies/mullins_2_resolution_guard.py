...
# Preserve close-time selection, but independently require payout within 48 hours.
...
def decide(ctx):
    ...
    # Cancelled orders remain busy until a later snapshot confirms removal.
    cancels = []
    for order in orders:
    ...
    thought = (f'Saw {len(markets)} open markets, {len(found)} favourites with a tight spread, enough volume and expected payout within 48h. '
