# Corrected child of krasker-3; family: options-pullback.
# Contract identity only: retain the parent's signals and pricing rules.
import math
import re
...
def option_symbol(row):
    if not isinstance(row, dict):
        return ""
    # A position's symbol can be its underlying, not its contract.
    symbol = str(row.get("occ") or row.get("symbol") or "").strip()
