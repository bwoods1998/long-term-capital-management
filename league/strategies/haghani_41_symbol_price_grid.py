# Corrected child of haghani-41; family: crypto-alts-reversion.
# Same alternative-coin maker-reversion specialty; starts with no record.
# Rest dip bids below the rolling mean and exit at the mean, with the
# parent's stop, holding-time limit and next-wake order replacement policy.
# Known venue grids govern limit prices; absent grids do not imply cents.
...
def _grid_price(ctx, symbol, price, side):
    """Snap to a stated grid without making the requested price more aggressive."""
    rule = ((ctx.get('venue_rules') or {}).get(symbol) or {})
    increment = _num(rule.get('price_increment'))
    if increment <= 0:
        return price
    step = Decimal(str(increment))
    rounding = ROUND_FLOOR if side == 'buy' else ROUND_CEILING
    units = (Decimal(str(price)) / step).to_integral_value(rounding=rounding)
...
def decide(ctx):
    ...
    window, k, replace_pct = max(2, int(_num(p.get("window"), 32))), _num(p.get("k"), 2.0), _num(p.get("replace_pct"), 0.227558)
    ...
    for symbol in NEEDS["symbols"]:
        ...
        exit_target = None if mean is None else _grid_price(ctx, symbol, mean, 'sell')
        ...
        if position is not None:
            ...
            if why:
                ...
                stale = [o for o in sells if _moved(o, exit_target, replace_pct)] + sells[1:]
                _cancel(cancels, stale)
                notes.append(f"{symbol}: holding, the sell at the mean {'is being moved' if stale else 'rests'} near {exit_target}")
            else:
                intents.append({"symbol": symbol, "side": "sell", "quantity": position["quantity"], "type": "limit", "limit_price": exit_target,
                                "reason": (f"Resting a limit sell of all {symbol} at {exit_target}, the grid-aware mean of the last {window} 15-minute closes: "
                                           f"bought at {cost:.2f}, the trade is over when the price is back at its mean.")})
                notes.append(f"{symbol}: holding, resting the exit at {exit_target}")
            continue
        ...
        if mean is None:
            ...
            continue
        target = _grid_price(ctx, symbol, min(mean - k * atr, mean * (1.0 - edge_pct / 100.0)), 'buy')
        if buys:
            stale = [o for o in buys if _moved(o, target, replace_pct)] + buys[1:]
            _cancel(cancels, stale)
            notes.append(f"{symbol}: the dip bid {'is being moved' if stale else 'rests'} near {target}")
            continue
        ...
        if age is None or age > STALE_HOURS or target <= 0 or bid <= 0 or target >= bid or dollars < 1.0:
            notes.append(f"{symbol}: no bid placed (dip price {target}, bid {bid:.2f}, ${max(dollars, 0.0):.2f} spendable)")
            continue
        free -= dollars
        intents.append({"symbol": symbol, "side": "buy", "notional_usd": dollars, "type": "limit", "limit_price": target,
                        "reason": (f"Resting a limit buy of ${dollars:.2f} of {symbol} at {target}: {(1.0 - target / mean) * 100.0:.1f}% under the "
                                   f"{window}-bar mean {mean:.2f} ({k:.1f} x the average range {atr:.2f}). It fills only on a sharp dip, as a maker; exit at the mean.")})
        notes.append(f"{symbol}: resting a dip bid at {target}")
