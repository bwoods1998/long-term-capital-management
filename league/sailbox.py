...
- **The key never leaves this module.** It is fetched through `default_key_source` (the
  `SAIL_API_KEY` environment variable, else the repository's owner-private `.env`) at the moment
  of the request, put in one `Authorization` header, and never logged, printed,
...
#: The repository (or release) root, where an owner-private `.env` may hold the Sail key.
REPO_ROOT = Path(__file__).resolve().parents[1]


def _key_from_env_file(path: Path) -> str | None:
    """Parse `SAIL_API_KEY=` out of an owner-private `.env`. None when unusable: a symlink, a file
    anyone but its owner can read, no such line or more than one."""
    try:
...
def default_key_source() -> str:
    ...
    if not key:
        raise SailboxError("provider_key_missing", kind="provider_key_missing")
    return key
