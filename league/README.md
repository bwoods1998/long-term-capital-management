...
| `verticals.py` | Level-3 debit verticals, the pure pieces only (Sept 23, 2026, workstream O: a design and a branch, not a deploy; `docs/design/2026-09-24-level-3-debit-verticals.md`). `DebitVertical` and `parse_vertical` (two legs of one underlying, expiry and right, the long leg the dearer strike; a credit spread is refused), `max_loss` (net debit x 100 x quantity), `cap_refusal`/`fits_caps` (maximum loss against the order and position caps), `mleg_body` (Alpaca's documented multi-leg order, which the gateway refuses) and `count_trades` (one spread's per-leg fills counted as ONE closed trade). **Imported by nothing in the House**, and `test_verticals` fails the day something does: a vertical's sold leg is a negative position the practice book refuses to adopt, so a spread on the shared practice account would freeze every Alpaca practice agent. |
