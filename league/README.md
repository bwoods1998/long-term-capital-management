...
| `options_replay.py` | The options desk's rung 0, reached from `replay.py` for a tape with `asset_class: option`: point-in-time listing, the 100 multiplier, an assumed $0.05 a contract a fill, limit orders filled only by later bars that trade a tick through (or at a recorded OPRA touch), liquidity limits, day orders, the 14:30 expiry offer and a write-off at zero. Uploaded into the box beside `replay.py`. |
