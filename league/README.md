...
| `structures.py` | Level-3 option structures with defined risk (the options-desk run, Sept 25, 2026): debit and credit verticals, iron condors and butterflies, long butterflies, calendars and diagonals, long straddles and strangles (`TYPES`; a naked short, an uncovered ratio, a broken wing or a less favourable diagonal is refused). `parse` reads a strategy's intent (natural limit: the most debit, the least credit); `classify` validates legs. A structure is HELD as ONE long option-class instrument (`instrument`, `market_id` = type and legs in canonical order; `is_structure`, `spec_of`) priced at S = net value + collateral K (zero for a debit structure, the widest wing for a credit one), so what it costs is its maximum loss, the caps meter it exactly, it is bought to open and sold to close, its mark is its bid (`quote`: all legs at once), a flat sale is one closed trade, and no book holds a negative leg. `intrinsic`, `fee_per_unit` ($0.05 a contract a leg) and `mleg_legs` (the venue's multi-leg legs) complete it. |
| `verticals.py` | The design's pure pieces for debit verticals (`docs/design/2026-09-24-level-3-debit-verticals.md`): `DebitVertical`/`parse_vertical`, `max_loss`, `cap_refusal`, `mleg_body` (Alpaca's documented multi-leg order) and `count_trades` (per-leg fills counted as one closed trade), the reference the practice book's leg adoption is tested against. Held positions use `structures.py`. |
...
| `options_shadow.py` | `OptionsShadowBroker` (Sept 25, 2026, the options-desk run's Track S): the `options-shadow` book's account, every level-3 structure on practice held as ONE position (`structures.py`), filled on live leg quotes (`alpaca_leg_quotes`: Alpaca's latest option quotes, every leg of a pass in one request). It trades structures only; an order never fills in the quote its decision saw, only in `advance()` on a strictly newer quote, an open at the structure's ask within its limit and a close at its bid, every leg two-sided, at most 10% of each leg's shown size on the side it trades over its ratio, in the regular session, day orders expired at the close. Fee `structures.fee_per_unit` a fill (`fees.py` charges the book the same for this venue). `structure_settlements` settles a structure still held past its earliest expiry at its value on the underlying's close (`alpaca_underlying_close`), when `Book.expire_options` asks, and the book books that value, never zero. Holds no credential and sends nothing. State in `options-shadow.json`. |
...
| `sensors.py` | `JevFloor`, the House's one hook for the Jev work below (`config.json` `jev`): `research_due` (from `House._gate`), `tick` (inactivity sweep; triage, links and exposure as background jobs while open for business, and the move sensor as its own `jev:move` job) and `health` (`health.json` `jev`, with `move`). |
| `jev_features.py` | `MoveSensor` (J1, Sept 25, 2026): every 5 minutes, point-in-time rows for every Kalshi market the strategies were shown (`recordings.sqlite` `markets:*` snapshots, read from the newest one at first start: nothing back-filled): `move_p5/15/60` from the free served model (`jev_move_model.json`, 23 features, no Jev), the lab-5 baseline, and a shadow `jev_p*` with Jev's six static answers asked once per market. `latest(market, at)` never returns a row recorded after `at`; `python -m league.jev_features evaluate --store ... --cutoff ...` is the held-out, post-ship evaluation. Not yet served to strategies. |
...
| `backup.py` | `Backup`: once a day the House checkpoints its own box with Sail (`league-<date>`, kept a week), on the ops lane; the `ops.deploy` rows with `what: backup` are the attempts. A failed try waits 30 minutes, doubling to 6 hours (`RETRY_SECONDS`, `RETRY_MAX_SECONDS`, Sept 24, 2026). Since H2 (Sept 25, 2026) a row that failed on Sail's side carries `environment: "sail"`, and `during_shutdown` when the House was shutting down as it came back; `notice` is what the House says (`House._run_backup`): a Sail outage is ONE error alert when it begins, a warning at each later backoff step and an info with its length (`outage_seconds`) when a checkpoint works again, all marked `environment` so the watchdog never rolls a release back for them; a failure of this code's own is an unmarked error every time, with `began_at`; one that came back during a shutdown is a warning. On Sept 24-25 Sail's checkpoint API answered 503 from 21:31:55Z to 01:55Z (73 failed tries) and six releases were rolled back on the backup's error alerts. |
| `__main__.py` | `python3 -m league run | tick | found | status | verify | stop | start`. On TERM the `run` loop ends after the tick in hand without sleeping out the rest of the tick's minute (H2, Sept 25, 2026: `time.sleep` sleeps through a signal, and TERM-to-exit took 24-82 s over the 13 restarts from 18:43Z Sept 24); `House.close` then waits at most `House.SHUTDOWN_WAIT_SECONDS` (5 s) for background work in flight, so a backup hung in Sail's checkpoint call never holds a restart, and one cut off writes nothing. |
| `watchdog.py` | `python3 -m league.watchdog deploy | status | rollback | prune`: releases on the box, the canary, promotion, the watch, the rollback. Imports nothing else from the package at import time. `read_health` reads `health.json` `failures` as bad readings (Sept 24, 2026), and with `inherited_before` (the watch passes the promotion's moment) counts a failure whose `since`, or an error alert whose `began_at`, is earlier as inherited, never a rollback: the lab's idle hour cannot roll back a release, a canary still refuses on it, `status` shows it. A frozen book in a `health.json` dated before the House's first `ops.started` since the promotion is the old process's (`frozen_by_previous_process`), not the release's. A vendor's outage is never a rollback (H2, Sept 25, 2026): an error alert carrying `environment` (the House marks one only where a call to a service outside its process failed on the service's side, `service_failed`: a 5xx, 408/425/429, a timeout, a refused/reset/unreachable connection, a DNS failure; never a TypeError, KeyError, ValueError, a 4xx or anything else of this code's own, wrapped or not) is counted in the reading's `detail.environment_alerts`, the first in `environment_first` (`{seq, service, text}`), in the watch and the canary alike, and never a reason. `environment(service, exc)` is the marker the House's call sites add: the backup (`sail`), the site publish (`site`), a replay box, a fork or retire and Sail's runway (`sail`), the frontier month and the profit-indexed raise (`gateway`), the underlier bars (`data`), and a failed background job (the job's name: `update` for GitHub, `feeds`, `research`, ...). Never on the trading path (an agent's wake, its box run, its market data, a venue's poll or settlement): a release can make a service fail there (a 429 storm, a shorter timeout), and those alerts are the only thing the watch sees of a release that stops every agent's trading, so they still roll back. `service_failed` judges a wrapper by what it was raised `from`, never by an exception it merely handled. A warning that repeats into an error (`House._repeating`) keeps the marker only when every warning of its run carried the same one. |
...
`House.tick()` is the whole loop (`python3 -m league run` calls it every `tick_seconds`, 30 since Sept 25,
2026). The tick serves the wakes; the steps beside them keep the cadence they had while ticks were a minute
apart (H5 of the forward-first run: measured on the box 04:31-05:00Z Sept 25, ticks of 19-100 s, p50 60.8 s, the
births pass 22.0 s of it at p50, and the log's tick lines 60-68 s apart at p50 in quiet hours).
...
5. **Every `mark_every_seconds` (300), per book:** poll, mark every account, reconcile to the venue
   ...
   `book.PRACTICE_DUST_USD`, and so are a real book's cents that its option and stock fills' unlisted
   regulatory fees explain, under `allocator.real_book_dust_usd`, with an error alert). Then judge each living agent on that
...
6. **Start due research passes** in the background, stuck agents first and then whoever has waited
   longest. Since Sept 25, 2026 the scheduling itself (`_schedule_research`: close a dead agent's
   saved session, ask `research_due` of every agent, queue the due) runs on the House's own lane
   once a minute, beside the tick (6.3 s of the tick at p50 on the box before, 24.0 s at its
   slowest). The base interval is three hours, or one for an agent that cannot act at all, and 15
...
7. **Start Merton's due roles** in the background, and ask the gateway what CI made of each open
   ...
   Then the **hypothesis foundry** (`hypotheses.py`), on the House's lane once a minute beside the
   tick since Sept 25, 2026 (`_foundry_step`, with a standings table of its own for the run). Each
   step labels new births, and every ten minutes it retires exhausted families. It makes at most one paid call per
...
9. **Keep the population:** kill agents at zero credits, rung-0 agents past the replay deadline, and
   ...
   **Dust and refused wind-downs** (Sept 24, 2026): a holding the venue will not trade (worth under a
   ...
   Since Sept 25, 2026 (H5) the births pass runs every `population_pass_seconds` (300), or on the tick
   after any birth or death (`_births_due`, on the registry's roster), never on every tick: at the
   ceiling with nobody displaceable it seated nobody and took 22.0 s of the tick at p50, the refill
...
10. **Save `house.json`, publish, write `health.json`.** A publishing failure is a warning: the site
    is downstream of the floor, never upstream. The site is checkpointed once a minute
    (`publish_seconds`), as at sixty-second ticks. `health.json` counts the restarts
    (`restarts_24h`, `restarts_24h_in_session`, `last_start`) and what became of the research in
    flight at this start (`restart_research`: `at_start`, `resumed`, `retired`, `lost_count`, `lost`,
    `waiting`). `health.json` `deferred` lists what the ticks of the
...
| `options-shadow.json` | The options shadow account (Sept 25, 2026): cash, structures held by code, orders (with the quote time each may fill after), fills, expiry settlements applied. |
...
| `jev.sqlite` | The Jev sensor's cached answers and every call's intent, status, cost and latency (what the daily caps are checked against), and a `days` summary that keeps lifetime spend exact after calls older than 35 days are pruned. |
| `jev-features.sqlite` | The move sensor's cursor, its own minute quotes, its per-market Jev labels and the point-in-time rows (14 days kept). |
