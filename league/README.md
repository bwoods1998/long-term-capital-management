...
| `engineer.py` | The repair engineer, Merton's sixth job with no more authority than the others: one paid patch a step for the top admitted job, a pull request through the same gateway route and path guard, CI's failure text read back (`GET /v1/github/pr/<n>/failures`) for at most `max_attempts` revisions, then `canary` (the running release holds the files), `observing` and `verified` only if the signal has not recurred. Strategy defects become new child strategies on rung 0, which `House.enroll` seats even in a full league (Sept 23, 2026); anything outside the role paths is `dormant: needs core authority`. Settings in `engineer.json` (owner-only). `scripts/repair_drill.py --plant` files a labeled synthetic job through the House's `repairs-inbox/` (the House stays the ledger's only writer); a drill asks no model and spends nothing. Each step leaves the queue in `<state>/repairs.json`. |
...
| `allocator.py` | Capital is the ladder (Sept 23, 2026). `evidence` computes `W_paper` (Alpaca paper haircut), `W_real`, `E` and the trade counts. `target_band`, `swing_stake` and `limits_for` are pure rules. `Allocator.rebalance` makes the mark pass's moves, stakes, deaths and performance fee inside the envelope and the throttle. `Allocator.board()` is what the publisher and the watch read. It is a money judge, in `ci.FORBIDDEN`. |
| `shards.py` | The Kalshi shard funder (Sept 23, 2026, U2). Kalshi runs markets on several exchange shards and an order on an unfunded one fails with `insufficient_shard_balance`. `ShardFunder.tick` runs on the tick (a cursor scan of new `book.order` rows: a real order refused that way is a warning alert naming the agent, market and shard, and a pass is requested); `ShardFunder.run` is the hourly pass on the ops lane: reads the cash per shard, works out the wanted shards from the `exchange_index` of the markets the living Kalshi desks are offered plus every shard holding a real position or resting order, and tops up each one under `FLOOR_USD` ($20) with `TOP_UP_USD` ($30) from the richest other shard that keeps its floor (`KEEP_USD` $60 on shard 0) and the stakes of the desks on it. Bounds: $100 a move, $200 a rolling day counted from the ledger; nothing under the kill switch, an inactive grant, a pause or a frozen real book; a failed move backs the shard off an hour. Every move is an `ops.alert` with a `shard_move` payload (transfer id, amounts, balances before and after). State in `shards.json`; `health.json` `shards`. A money mover, in `ci.FORBIDDEN`. |
...
7. **Start Merton's due roles** in the background, and ask the gateway what CI made of each open
   ...
   House is open for business and the lab is open (above all, the OpenAI tier is `all`), the tick
   schedules one step on the replay lane (`replay:lab:step`) and does no lab work itself. A step
   seeds, breeds, evaluates batches on the lab box for up to `step_seconds` (240), graduates, pays
   royalties and writes `lab.stats`.
...
| `shards.json` | The Kalshi shard funder's state (Sept 23, 2026): the `book.order` cursor of its refusal scan, the last check, the learned series-to-shard map, the last balances per shard, the wanted shards, the shards backing off after a failed move, and the rolling day's moved total. A lost file re-reads the balances and re-learns the map at the next pass; the day's cap is counted from the ledger, not from here. |
