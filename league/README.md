...
| `kalshi_founders.py` | K1 (the Kalshi-scale run, Sept 25, 2026): `seat(house)`, called once a births pass right after `enroll`, births at most one founder row of an open Kalshi desk flagged `"seat_full_league": true` that was never born (`House.found`, rung 1), making room through the House's own displacement rules: on its own desk when that desk is full, else in a full league from the desk whose pooled forward record over the last 7 days (`desk_records`: `family_forward`'s active `eval.block` growth, by desk) is the most negative, never a desk whose record is not negative; the resident dies `displaced` naming the founder, the birth writes its `birth-route` row and an info alert, and a refusal is `seat_refusals.founders` with one warning an hour. It never raises. |
