...
| `kalshi_founders.py` | K1 (the Kalshi-scale run, Sept 25, 2026): `seat(house)`, called once a births pass after `enroll` under the House's lifecycle lock, births at most one founder row of an open Kalshi desk flagged `"seat_full_league": true` that was never born (`House.found`, rung 1). Room comes from the House's own seat market first (its own desk when that desk is full; else, in a full league, the desk whose pooled forward record over the last 7 days, `desk_records`, is the most negative, never one whose record is not negative; the resident dies `displaced`), then from a desk whose niches.json row carries `"yields_seats": {"floor", "reason"}` (the fifteen-minute crypto desk, down to 4): its weakest practice resident that is flat, undrained, off real money and of no proven, swinging or the founder's own family dies `desk_closed`, never counted as a displacement. Every birth writes its `birth-route` row and an info alert; a refusal is `seat_refusals.founders` with one warning an hour. It never raises. |
