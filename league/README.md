...
| `lab.sqlite` | The Alpha Lab's archive: `candidates`, `archive` (one elite a cell), `batches`, `calls`, `graduations`, `forward` (one row per candidate and forward-window run, S2) and `meta` (Sept 23, 2026). A lost file means an empty archive, searched again from the seeds; the ledger keeps the `lab.*` rows. |
