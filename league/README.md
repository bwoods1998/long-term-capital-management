...
| `verticals.py` | Level-3 debit verticals (`docs/design/2026-09-24-level-3-debit-verticals.md`; the options-desk run of Sept 25, 2026). `DebitVertical` and `parse_vertical` (two legs of one underlying, expiry and right, the long leg the dearer strike; a credit spread is refused and cannot be represented), `max_loss` (net debit x 100 x quantity), `cap_refusal`/`fits_caps`, `mleg_body` (Alpaca's documented multi-leg order) and `count_trades` (per-leg fills counted as ONE closed trade). A spread is HELD as one option-class instrument priced at its net (`spread_instrument`, `market_id` `LONG/SHORT` in OCC codes; `is_spread`, `legs_of`, `vertical_of`, `spread_quote`, `intrinsic`): what it costs is its maximum loss, it is only ever bought to open and sold to close, and a sale that leaves it flat is one closed trade, so no book ever holds a negative leg. |
