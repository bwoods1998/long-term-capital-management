...
| `commons.py` | `Commons`: web search (Sail's search API, Google News RSS as fallback, $0.01 charged per query), web fetch (one public page's text read by the gateway's `/v1/web/fetch`, never by the House box; `gateway_fetch`), the research library, the tool-request queue (ordered by demand; unresolved and blocked engineering requests remain visible until explicitly resolved), the playbook. All of it is ledger rows. |
| `researcher.py` | `Researcher`: a cheap model's tool loop for one agent (a Sail model, or GPT-6 Luna through the gateway for the share of sessions `fast_research.ResearchRouter` sends there), every token and tool call charged to that agent. Its `replay` tool is a counted trial. Its `web_fetch` tool (I1, Sept 25, 2026) reads one public page through the gateway, charged like a search, at most 20 a UTC day per agent (counted from its `agent.research` rows, tool `web_page`), and hands the model the text as untrusted data between two marker lines named by a random nonce (the marker's words are removed from the page itself). A read the gateway may have made is charged and counted, a Worker error or a timeout included; one refused before any read is free. A passing candidate is adopted on rung 0 and forked above it. A session the provider failed (`research_gate.provider_fault`: HTTP 500, 502, 503, 504, 529) has its research-token charges refunded in one `credit.grant` (`refund_provider_fault`, Sept 24, 2026). |
...
| `seeds/` | The fourteen founding strategy files (Kalshi, crypto, equity and options) and `all_seeds()`. They are data, not modules: nothing imports them. Since Sept 25, 2026 (K3 of the Kalshi-scale run) `weather_ensemble.py` is seated as four more rows, `weather-ensemble-east`, `-central`, `-texas` and `-west`, founders of the weather desk with a family each: every daily high and low bracket of their stations priced from the `weather` feed's ensemble members, calibrated to Kalshi's own settlements (`tests/fixtures/weather_ensemble/station_calibration_20260924.json`), shrunk toward the market mid, entered as post-only maker bids before the day's own observations. |
...
| `kalshi_founders.py` | K1 (the Kalshi-scale run, Sept 25, 2026): `seat(house)`, one call line in the births pass after `enroll`, births at most one founder row of an open Kalshi desk flagged `"seat_full_league": true` that was never born, in `seat_priority` order (lower first, then desk, then row), through `House.found` (rung 1), and only into a free seat of its own desk (its desk full, it waits). The seat question, the birth and the death that makes room are one step under the House's lifecycle lock; the founder's NEEDS probe is read before it (`found(described=...)`) and the question asked again after. In a full league room comes from the House's own seat market first, asked as for a newcomer without forward evidence (`evidenced=False`), on the desk whose pooled forward record over the last 7 days (`desk_records`: blocks by when they began, folded after a cursor, kept an hour) is the most negative, never one whose record is not negative (the resident dies `displaced`); then from a desk whose niches.json row carries `"yields_seats": {"floor", "reason"}` (the fifteen-minute crypto desk, down to 4), whose weakest practice resident dies `desk_closed`, never counted as a displacement. On both paths the resident is never on real money, of a proven or swinging family, holding a position or a working order, drained, held for the open or researching. The founder is born before the resident dies; if `found` refuses or raises, nobody dies. Every birth writes its `birth-route` row and an info alert; a refusal is `seat_refusals.founders` with one warning an hour; a malformed row is skipped and told. It never raises. |
