...
| `triage.py` | `Triage`: tool requests, post-mortems, abstentions and journals/thoughts become deduplicated `repair.reported` rows (`source: triage`) with their evidence and affected agents. Grouping is exact first; Jev decides "same missing feed?" (merge only at high confidence) and "does this report a bug?". |
