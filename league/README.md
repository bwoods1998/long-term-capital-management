...
5. **Every `mark_every_seconds` (300), per book:** poll, mark every account, reconcile to the venue
   ...
   **Since Sept 23, 2026, while `allocator.enabled`, capital is the ladder.** After the books are
   ...
   1. Reads every agent's evidence: `W_paper`, `W_real` and `E` (`Evaluator.wealth`), and each
      family's pooled record once (`family_record`, through the pass's ledger position).
   ...
   5. Moves bands up, best E first, inside the envelope: a probe when the agent's family is not
      proven, a bunt when it is. A known defect is audited before the bunt, and the first swing is
      audited.
   6. Sizes every real stake toward its target (a probe's or a bunt's, by the family's state).
