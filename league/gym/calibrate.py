...
half-spread; a sell mirrors it). The exposure is every quoted contract-minute of the sampled
contracts, from the same day's NBBO file; the prints are the day's `trade_quote` rows, each placed by
...
that real and paper fills replace (the plan: "then from real fills").
...
def fit(store: Store, roots: Sequence[str], *, days: Sequence[dt.date] | None = None, min_exposure: int = 300) -> dict:
    ...
    for root in roots:
        ...
        for day in candidates:
            ...
            ok = (idx >= 0) & (minute >= 1) & (minute < chain.minutes - 1) & (ask > bid) & (bid >= 0) & (price > 0)
            ...
            # Exposure: every quoted minute of the sampled contracts.
            sampled = np.unique(idx)
            valid = ~np.isnan(chain.bid[:, sampled])            # [M, C']
            spot = chain.underlying.price
            mm, cc = np.nonzero(valid[1:-1])
            mm = mm + 1
            contract = sampled[cc]
            money = chain.strike[contract] / spot[mm] - 1.0
            cell = _cells(chain.dte[contract], np.nan_to_num(money), chain.open_min + mm)
            ...
            pcell = _cells(chain.dte[idx], np.nan_to_num(pmoney), chain.open_min + minute)
    ...
    for code, n in exposure.items():
        ...
        for q in LEVELS:
            ...
            for cls, p in (("s", single), ("m", multi)):
                if p > 0:
                    hazard[f"q{q}|{cls}|d{d}|k{k}|t{t}"] = round(p, 6)
