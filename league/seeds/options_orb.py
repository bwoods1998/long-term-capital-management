...
NEEDS = {"venue": "alpaca", "horizon": "day", "style": "options-orb", "asset_class": "option", "structures": True,
         ...
         "parameter_rules": {"bounds": {"width": [1, 5], "dte_min": [0, 4], "dte_max": [0, 4], "entry_delta": [0.2, 0.6], "profit_target": [0.2, 0.95],
...
PARAMS = {"structure": "debit_vertical", "width": 1.0, "dte_min": 0, "dte_max": 4, "entry_delta": 0.45, "profit_target": 0.5, "stop_loss": 0.5,
          "exit_minutes_before_close": 25, "exit_dte": 99, "max_open": 2, "max_qty": 1, "notional_usd": 60.0, "slip": 0.01, "max_debit": 0.65,
...
def decide(ctx):
    ...
    return {"intents": intents[:8], "cancels": cancels[:20], "thought": "Options ORB. " + ("; ".join(notes) or "nothing to do") + ".", "memory": {"done": done}}
