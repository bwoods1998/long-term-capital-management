# favorites-daily: rest post-only YES bids on daily weather and commodity favourites priced 90 to
# 97 cents. The same logic as favorites-maker, pointed at the groups where the edge was measured.
#
# THE IDEA. On Kalshi the side that waits is paid by the side that hurries, and favourites are
# underpriced while longshots are overpriced. Find a daily market where YES is already the strong
# favourite (today's high in a city, where oil or gold settles), join the best YES bid as a maker,
# and hold the contract to settlement.
#
# THE EVIDENCE. Across 72 million Kalshi trades takers lose about 1.1% and makers earn it. The
# first run's ONE measured edge: resting maker bids on favourites above 90 cents made +2.15 cents
# a contract out of sample (interval +1.62 to +2.72), and by group it was the WEATHER and
# COMMODITY favourites that were positive (crypto favourites were negative on a small sample).
# Weather series pay no maker fee. This is the seed with the best evidence behind it.
#
# WHAT IT NEEDS. The open markets of seven city daily-high series plus daily WTI and gold, with
# the YES touch, hours to close and the day's volume. No bars, no memory. Woken hourly: a daily
# market moves slowly and a resting bid does the waiting.
#
# WHEN IT TRADES. A market qualifies when its YES bid is between bid_min and bid_max, the spread
# is at most max_spread, it closes in min_hours to max_hours (up to 30) and volume_24h is at
# least min_volume_24h. It rests a post-only YES bid AT the current YES bid (never above it, so it
# can never cross the ask), sized notional_usd in whole contracts. At most max_open markets at
# once, counting holdings and resting orders, and never a second order in a market it is already
# in. The busiest markets are taken first.
#
...
NEEDS = {
    ...
    "series": ["KXHIGHNY", "KXHIGHCHI", "KXHIGHMIA", "KXHIGHAUS", "KXHIGHDEN", "KXHIGHLAX", "KXHIGHPHIL", "KXWTI", "KXGOLDD"],
