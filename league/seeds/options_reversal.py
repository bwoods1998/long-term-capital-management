...
NEEDS = {"venue": "alpaca", "horizon": "day", "style": "options-reversal", "asset_class": "option", "structures": True,
         ...
         "parameter_rules": {"bounds": {"width": [1, 5], "dte_min": [1, 7], "dte_max": [1, 7], "entry_delta": [0.2, 0.6], "profit_target": [0.2, 0.95],
...
PARAMS = {"structure": "debit_vertical", "width": 1.0, "dte_min": 2, "dte_max": 7, "entry_delta": 0.45, "profit_target": 0.5, "stop_loss": 0.6,
...
def decide(ctx):
    ...
    return {"intents": intents[:8], "cancels": cancels[:20], "thought": "Options reversal. " + ("; ".join(notes) or "nothing to do") + ".",
            "memory": {"done": done, "events": {k: v for k, v in events.items() if k in NEEDS["symbols"]}}}
