# options-strangle-cheap: buy a long strangle (or straddle) on IWM, QQQ or SPY, 2-5 days to expiry,
# only when the options price a SMALLER move than the underlying has been making, and sell it on a
# move or after a day.
#
...
# this replay.
#
# WHAT IT NEEDS. The chain (2-5 days) and hourly bars of the underlyings.
#
...
# the strikes in dollars; 0 lets a straddle through); dte_min 1-4, dte_max 2-7; entry_delta 0.1-0.5
...
# vol_days 3-20; vol_cushion 0.6-1.2; min_edge 0-0.2; risk_usd 20-75; requote_minutes 5-60;
...
NEEDS = {
    ...
    "wake_minutes": 10,
    "parameter_rules": {
        "bounds": {"width": [0, 10], "dte_min": [1, 4], "dte_max": [2, 7], "entry_delta": [0.1, 0.5], "profit_target": [0.1, 1.5],
                   "stop_loss": [0.1, 0.8], "exit_minutes_before_close": [5, 120], "max_open": [1, 3], "entry_start": [570, 930],
                   "entry_end": [570, 930], "cheap_max": [0.5, 1.2], "vol_days": [3, 20], "vol_cushion": [0.6, 1.2], "min_edge": [0.0, 0.2],
                   "risk_usd": [20, 75], "requote_minutes": [5, 60], "max_hold_days": [1, 3], "max_entries_day": [1, 2]},
        "ordered": [["dte_min", "dte_max"]],
    },
}
PARAMS = {"structure": "long_strangle", "width": 1.0, "dte_min": 2, "dte_max": 5, "entry_delta": 0.2, "profit_target": 0.4,
          "stop_loss": 0.4, "exit_minutes_before_close": 30, "max_open": 2, "entry_start": 600, "entry_end": 900,
          "cheap_max": 0.9, "vol_days": 10, "vol_cushion": 0.9, "min_edge": 0.0, "risk_usd": 75.0, "requote_minutes": 10,
    ...
    for expiry in _expiries(tree, symbol, ny.date(), int(p["dte_min"]), int(p["dte_max"])):
        ...
        for dc, sc, call in sorted(legs_call)[:6]:
            for dp, sp, put in sorted(legs_put)[:6]:
                ...
                edge = _edge(price, rows, spot, real * p["vol_cushion"])
                if edge is None or edge < p["min_edge"] or _open_limit(price) is None or not _fits(price, 1, ctx, p["risk_usd"]):
                    continue
                score = (dc + dp, -edge)
        ...
        reason = (f"Buying the {symbol} {expiry} {strikes} {kind.replace('_', ' ')} for {limit:.2f} (at the touches): the implied move to "
                  f"expiry {implied:.2%} is only {ratio:.2f}x the {real:.2%} the last {int(p['vol_days'])} sessions imply; expected "
                  f"{edge:+.3f} a share at expiry after costs; at most ${price['held_open'] * 100 * quantity:.0f} can be lost. Out at "
