...
# entry_start/entry_end 570-930 (minutes after midnight New York); vrp_min 0.8-2; vol_days 3-15
# (sessions of realized volatility); vol_cushion 0.8-2; min_edge 0-0.2 ($ a share); risk_usd 20-75;
# requote_minutes 5-60; max_entries_day 1-6.
...
NEEDS = {
    ...
    "parameter_rules": {
        "bounds": {"width": [1, 5], "dte_min": [0, 2], "dte_max": [0, 7], "entry_delta": [0.05, 0.35], "profit_target": [0.2, 0.9],
                   ...
                   "entry_end": [570, 930], "vrp_min": [0.8, 2.0], "vol_days": [3, 15], "vol_cushion": [0.8, 2.0], "min_edge": [0.0, 0.2],
                   "risk_usd": [20, 75], "requote_minutes": [5, 60], "max_entries_day": [1, 6]},
...
PARAMS = {"structure": "iron_condor", "width": 1.0, "dte_min": 0, "dte_max": 2, "entry_delta": 0.15, "profit_target": 0.5,
          ...
          "vrp_min": 1.1, "vol_days": 5, "vol_cushion": 1.1, "min_edge": 0.01, "risk_usd": 75.0, "requote_minutes": 10,
          "max_entries_day": 2}
...
def _sides(tree, symbol, expiry, spot, ny, iv, p, sd):
    ...
    for right in ("put", "call"):
        ...
        for strike, row in rows.items():
            ...
            if price["open"] <= 0:
                continue
            out[right].append((abs(abs(delta) - p["entry_delta"]), _edge(price, legs, spot, sd), price, legs, delta))
    return out
    ...
    for expiry in _expiries(tree, symbol, ny.date(), int(p["dte_min"]), int(p["dte_max"])):
        ...
        for put in sides["put"]:
            for call in sides["call"]:
                ...
                if edge < p["min_edge"] or _open_limit(price) is None or not _fits(price, 1, ctx, p["risk_usd"]):
                    continue
        ...
        reason = (f"Selling the {symbol} {expiry} {strikes} iron condor for {limit:.2f} (at the touches): the implied move to expiry "
