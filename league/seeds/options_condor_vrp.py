# options-condor-vrp: sell a $1-winged iron condor on SPY, QQQ or IWM, 0-2 days to expiry, when the
# options price a bigger move than the underlying has been making (the variance risk premium).
#
# THE IDEA. Index options are priced above the volatility that follows on average: sellers are paid
# to insure. A short strangle harvests that but its loss is unbounded; an iron condor (a put credit
# vertical under the money plus a call credit vertical over it, one expiry, $1 wings) keeps the same
# bet with the loss capped at the wing less the credit. Sell it only when the at-the-money implied
# move to expiry is at least `vrp_min` times the move the underlying's own recent bars imply, and
# only when the condor's expected value at expiry under THAT realized distribution (fat-tailed, and
# widened by `vol_cushion`) clears the touch paid on every leg and the fee on both fills.
#
# THE EVIDENCE. Published: the variance risk premium on index options is positive on average and
# largest at short maturities; it is also what blows up in a crash, which the wings cap here. This
# firm has measured nothing yet: the replay on the options history store is its first look.
#
# WHAT IT NEEDS. The chain (0-2 days), 15-minute bars of the underlyings for realized volatility.
#
# WHEN IT TRADES. From `entry_start` to `entry_end` New York (and never on an expiry day from 14:30),
# at most `max_entries_day` condors an underlying a day and `max_open` at once, opened at the touch
# (long legs at the ask, short legs at the bid), sized to `risk_usd` of maximum loss inside the caps.
#
# HOW IT EXITS. At the touch: when it has made `profit_target` of its credit, when buying it back
# costs `stop_loss` times the credit, and `exit_minutes_before_close` before the close of its expiry
# day (before the House's own 15:30 close). Nothing is held into an expiry.
#
# PARAMS (ranges): width 1-5 ($ a wing); dte_min 0-2 and dte_max 0-7 (calendar days); entry_delta
# 0.05-0.35 (the short strikes' |delta|); profit_target 0.2-0.9 (of the credit); stop_loss 1.2-4
# (buy-back cost as a multiple of the credit); exit_minutes_before_close 5-120; max_open 1-4;
# entry_start/entry_end 570-930 (minutes after midnight New York); vrp_min 0.8-2; vol_days 3-7
# (sessions of realized volatility); vol_cushion 0.8-2; min_edge 0-0.2 ($ a share); risk_usd 20-75;
# requote_minutes 5-60; slip 0-0.2 ($ a contract a leg the entry's limit pays over the touch, and a profit target's under it); max_entries_day 1-6.
...
NEEDS = {
    ...
    "wake_minutes": 5,
    "parameter_rules": {
        "bounds": {"width": [1, 5], "dte_min": [0, 2], "dte_max": [0, 7], "entry_delta": [0.05, 0.35], "profit_target": [0.2, 0.9],
                   "stop_loss": [1.2, 4.0], "exit_minutes_before_close": [5, 120], "max_open": [1, 4], "entry_start": [570, 930],
                   "entry_end": [570, 930], "vrp_min": [0.8, 2.0], "vol_days": [3, 7], "vol_cushion": [0.8, 2.0], "min_edge": [0.0, 0.2],
                   "risk_usd": [20, 75], "requote_minutes": [5, 60], "slip": [0.0, 0.2], "max_entries_day": [1, 6]},
        "ordered": [["dte_min", "dte_max"]],
    },
}
PARAMS = {"structure": "iron_condor", "width": 1.0, "dte_min": 0, "dte_max": 2, "entry_delta": 0.15, "profit_target": 0.5,
          "stop_loss": 2.0, "exit_minutes_before_close": 30, "max_open": 2, "entry_start": 600, "entry_end": 840,
          "vrp_min": 1.1, "vol_days": 5, "vol_cushion": 1.1, "min_edge": 0.01, "risk_usd": 75.0, "requote_minutes": 10, "slip": 0.02,
          "max_entries_day": 2}


def _sides(tree, symbol, expiry, spot, ny, iv, p, sd):
    """Every out-of-the-money credit vertical `width` wide on each side whose short |delta| lies near
    `entry_delta`: (distance from the target delta, expected edge, price, rows, delta)."""
    out = {"put": [], "call": []}
    low, high = 0.4 * p["entry_delta"], min(0.45, 2.5 * p["entry_delta"])
    for right in ("put", "call"):
        rows = (tree.get(symbol) or {}).get(expiry, {}).get(right) or {}
        for strike, row in rows.items():
            if (right == "put" and strike >= spot) or (right == "call" and strike <= spot) or row["bid"] <= 0:
                continue
            delta = _delta(row, spot, ny, iv)
            if delta is None or not low <= abs(delta) <= high:
                continue
            wing = rows.get(round(strike - p["width"] if right == "put" else strike + p["width"], 3))
            if wing is None:
                continue
            ...
            if price["open"] <= 0:
                continue
            out[right].append((abs(abs(delta) - p["entry_delta"]), _edge(price, legs, spot, sd, p["slip"]), price, legs, delta))
    return out
...
def _entry(ctx, p, ny, tree, symbol, notes):
    ...
    for expiry in _expiries(tree, symbol, ny.date(), int(p["dte_min"]), int(p["dte_max"])):
        ...
        if not real or not implied:
            notes.append(f"{symbol} {expiry}: no {'realized' if not real else 'implied'} volatility")
            continue
        ratio = implied / real
        if ratio < p["vrp_min"]:
            notes.append(f"{symbol} {expiry}: implied move {implied:.2%} is {ratio:.2f}x the realized {real:.2%}, under {p['vrp_min']:.2f}")
            return None
        sides = _sides(tree, symbol, expiry, spot, ny, iv, p, real * p["vol_cushion"])
        best = None
        for put in sides["put"]:
            for call in sides["call"]:
                rows = put[3] + call[3]
                price = _price("iron_condor", rows)
                if put[1] is None or call[1] is None or price["open"] <= 0:
                    continue
                edge = put[1] + call[1]  # touches, payoffs and fees all add: the condor is the two verticals
                if edge < p["min_edge"] or _open_limit(price, p["slip"]) is None or not _fits(price, 1, ctx, p["risk_usd"], _held_at(price, _open_limit(price, p["slip"]))):
                    continue
                score = (put[0] + call[0], -edge)
                if best is None or score < best[0]:
                    best = (score, price, rows, edge, put[4], call[4])
        if best is None:
            notes.append(f"{symbol} {expiry}: rich ({ratio:.2f}x) but no {p['width']:.0f}-wide condor near {p['entry_delta']:.2f} delta clears "
                         f"the touch and fits ${p['risk_usd']:.0f} of risk")
            return None
        _, price, rows, edge, put_delta, call_delta = best
        limit = _open_limit(price, p["slip"])
        quantity = _size(price, ctx, p["risk_usd"], held=_held_at(price, limit))
        strikes = "/".join(f"{r['strike']:g}" for r, _, _ in rows)
        reason = (f"Selling the {symbol} {expiry} {strikes} iron condor for {limit:.2f} (the touches and a slip): the implied move to expiry "
                  f"{implied:.2%} is {ratio:.2f}x the {real:.2%} the last {int(p['vol_days'])} sessions imply; short deltas "
                  f"{put_delta:+.2f}/{call_delta:+.2f}; expected {edge:+.3f} a share at expiry after costs; at most "
                  f"${_held_at(price, limit) * 100 * quantity:.0f} can be lost. Out at {p['profit_target']:.0%} of the credit, "
                  f"{p['stop_loss']:.1f}x it, or {int(p['exit_minutes_before_close'])} minutes before its close.")
        notes.append(f"{symbol} {expiry}: selling {strikes} for {limit:.2f}, {ratio:.2f}x rich, edge {edge:+.3f}")
        return _intent("iron_condor", rows, "open", quantity, limit, reason)
    notes.append(f"{symbol}: no expiry {int(p['dte_min'])}-{int(p['dte_max'])} days out in the chain")
    return None
...
def decide(ctx):
    ...
    if ny is None:
        return {"intents": [], "cancels": [], "thought": "Condor VRP: no clock.", "memory": {}}
    ...
    if not _open_day(ny.date()) or not p["entry_start"] <= clock < p["entry_end"]:
        return {"intents": intents[:8], "cancels": cancels[:20], "memory": memory,
                "thought": ("Condor VRP. No entries now (entries " f"{int(p['entry_start']) // 60:02d}:{int(p['entry_start']) % 60:02d}-"
                            f"{int(p['entry_end']) // 60:02d}:{int(p['entry_end']) % 60:02d} New York). " + "; ".join(notes))[:1500]}
    ...
    for symbol in NEEDS["symbols"]:
        ...
        if intent is not None:
            ...
            slots -= 1
    return {"intents": intents[:8], "cancels": cancels[:20], "thought": ("Condor VRP. " + "; ".join(notes) + ".")[:1500], "memory": memory}
