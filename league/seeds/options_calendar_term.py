# options-calendar-term: buy a calendar (sell the near expiry, buy a later one at the same strike)
# when the near expiry's implied volatility is kinked above the later one's, and close it before the
# near expiry.
#
# THE IDEA. The term structure of implied volatility is usually upward sloping or flat. When the
# front is bid over the back -- an event priced into the next day or two, or a burst of fear -- the
# front's volatility tends to fall back toward the back's as the days pass, and the front leg also
# decays fastest. A calendar (short the near contract, long the far one, one strike, one right) is
# long that normalisation, and its loss is capped at the debit provided it is closed before the near
# expiry (the House closes it from 15:30 New York on that day; this strategy leaves earlier). Enter
# only when front IV / back IV >= `kink_min` and the calendar's expected value at the near expiry --
# the far leg valued at its own implied volatility over the underlying's realized distribution, less
# the near leg's intrinsic value -- clears the touch paid, the touch to close and the fees.
#
# THE EVIDENCE. Published: implied volatility term structure mean-reverts; inverted fronts
# normalise after events. Unmeasured by this firm before this replay.
#
# WHAT IT NEEDS. The chain out to `back_max` days (near and far expiries) and 15-minute bars.
#
# WHEN IT TRADES. From `entry_start` to `entry_end` New York (never on the near expiry's own day),
# one calendar an underlying a day, `max_open` at once, at the touch, sized to `risk_usd` of debit.
#
# HOW IT EXITS. At the touch: up `profit_target` of the debit, down `stop_loss` of it,
# `exit_minutes_before_close` before the close of the near expiry's day, or at that minute of the
# session after `max_hold_days` sessions.
#
# PARAMS (ranges): width 0 (a calendar has one strike; kept for the lab's common vocabulary);
# dte_min 1-3 and dte_max 1-4 (the near expiry); back_min 4-9 and back_max 6-14 (the far expiry);
# entry_delta 0.2-0.5 (the strike's |delta|); profit_target 0.1-1 (of the debit); stop_loss 0.1-0.8 (of
# the debit); exit_minutes_before_close 30-120; max_open 1-3; entry_start/entry_end 570-930;
# kink_min 0.9-1.6; vol_days 3-7; vol_cushion 0.8-1.5; min_edge 0-0.2; risk_usd 20-75;
# requote_minutes 5-60; slip 0-0.2 ($ a contract a leg the entry's limit pays over the touch, and a profit target's under it); max_hold_days 1-3; max_entries_day 1-2.
...
NEEDS = {
    "venue": "alpaca",
    "horizon": "day",
    "style": "options-calendar-term",
    "asset_class": "option",
    "structures": True,
    "symbols": ["IWM", "F", "SOFI", "INTC", "PFE", "T"],
    "bars": {"timeframe": "15Min", "limit": 500},
    "max_days_to_expiry": 10,
    "wake_minutes": 10,
    "parameter_rules": {
        "bounds": {"width": [0, 0], "dte_min": [1, 3], "dte_max": [1, 4], "back_min": [4, 9], "back_max": [6, 14], "entry_delta": [0.2, 0.5],
                   "profit_target": [0.1, 1.0], "stop_loss": [0.1, 0.8], "exit_minutes_before_close": [30, 120], "max_open": [1, 3],
                   "entry_start": [570, 930], "entry_end": [570, 930], "kink_min": [0.9, 1.6], "vol_days": [3, 7],
                   "vol_cushion": [0.8, 1.5], "min_edge": [0.0, 0.2], "risk_usd": [20, 75], "requote_minutes": [5, 60], "slip": [0.0, 0.2],
                   "max_hold_days": [1, 3], "max_entries_day": [1, 2]},
        "ordered": [["dte_min", "dte_max"], ["back_min", "back_max"]],
    },
}
PARAMS = {"structure": "calendar", "width": 0, "dte_min": 1, "dte_max": 3, "back_min": 5, "back_max": 10, "entry_delta": 0.45,
          "profit_target": 0.3, "stop_loss": 0.35, "exit_minutes_before_close": 45, "max_open": 2, "entry_start": 630,
          "entry_end": 900, "kink_min": 1.1, "vol_days": 5, "vol_cushion": 1.0, "min_edge": 0.0, "risk_usd": 75.0,
          "requote_minutes": 10, "slip": 0.02, "max_hold_days": 1, "max_entries_day": 1}


def _calendar_edge(front, back, spot, ny, sd, debit):
    """Expected value a share at the near expiry's close: the far leg at its own implied volatility,
    less the near leg's intrinsic value, over the realized distribution; less the debit, half of
    both legs' spreads to close, and the fees of both fills."""
    vol = back["iv"] or _implied((back["bid"] + back["ask"]) / 2.0, spot, back["strike"], _years(back["expiry"], ny), back["right"])
    if not vol or not sd:
        return None
    near_close = datetime(*[int(x) for x in front["expiry"].split("-")], 16, 0, tzinfo=NY)
    left = _years(back["expiry"], near_close)
    value = 0.0
    for z, w in zip(GRID, WEIGHTS):
        price = spot * math.exp(sd * z - 0.5 * sd * sd)
        inside = price - front["strike"] if front["right"] == "call" else front["strike"] - price
        value += w * (_bs(price, back["strike"], left, vol, back["right"]) - max(0.0, inside))
    exit_cost = 0.5 * ((back["ask"] - back["bid"]) + (front["ask"] - front["bid"]))
    return value - debit - exit_cost - 2.0 * FEE * 2 / 100.0
...
def _entry(ctx, p, ny, tree, symbol, notes):
    ...
    if not spot or profile is None:
        ...
        return None
    today = ny.date()
    fronts = [e for e in _expiries(tree, symbol, today, int(p["dte_min"]), int(p["dte_max"])) if _date(e) > today]
    backs = _expiries(tree, symbol, today, int(p["back_min"]), int(p["back_max"]))
    if not fronts or not backs:
        notes.append(f"{symbol}: no near ({int(p['dte_min'])}-{int(p['dte_max'])} days) and far ({int(p['back_min'])}-{int(p['back_max'])}) expiries")
        return None
    front_expiry = fronts[0]
    back_expiry = min(backs, key=lambda e: abs((_date(e) - _date(front_expiry)).days - 7))
    iv_front, iv_back = _atm_iv(tree, symbol, front_expiry, spot, ny), _atm_iv(tree, symbol, back_expiry, spot, ny)
    if not iv_front or not iv_back:
        notes.append(f"{symbol}: no at-the-money volatility for {front_expiry} or {back_expiry}")
        return None
    kink = iv_front / iv_back
    if kink < p["kink_min"]:
        notes.append(f"{symbol}: {front_expiry} at {iv_front:.1%} against {back_expiry} at {iv_back:.1%} ({kink:.2f}x; a kink is {p['kink_min']:.2f}x)")
        return None
    sd = _move_sd(profile, ny, front_expiry)
    sd = sd * p["vol_cushion"] if sd else None
    best = None
    for right in ("call", "put"):
        near = (tree.get(symbol) or {}).get(front_expiry, {}).get(right) or {}
        far = (tree.get(symbol) or {}).get(back_expiry, {}).get(right) or {}
        for strike in near:
            if strike not in far or near[strike]["bid"] <= 0:
                continue
            delta = _delta(far[strike], spot, ny, iv_back)
            if delta is None or abs(abs(delta) - p["entry_delta"]) > 0.2:
                continue
            rows = [(near[strike], -1, 1), (far[strike], 1, 1)]
            price = _price("calendar", rows)
            if price["open"] <= 0 or _open_limit(price, p["slip"]) is None or not _fits(price, 1, ctx, p["risk_usd"], _held_at(price, _open_limit(price, p["slip"]))):
                continue
            edge = _calendar_edge(near[strike], far[strike], spot, ny, sd, price["open"] + 2 * p["slip"])
            if edge is None or edge < p["min_edge"]:
                continue
            score = (abs(abs(delta) - p["entry_delta"]), -edge)
            if best is None or score < best[0]:
                best = (score, price, rows, edge, strike, right)
    if best is None:
        notes.append(f"{symbol}: kinked ({kink:.2f}x), but no calendar near {p['entry_delta']:.2f} delta clears the costs and fits")
        return None
    _, price, rows, edge, strike, right = best
    limit = _open_limit(price, p["slip"])
    quantity = _size(price, ctx, p["risk_usd"], held=_held_at(price, limit))
    reason = (f"Buying the {symbol} {strike:g} {right} calendar ({front_expiry} sold, {back_expiry} bought) for {limit:.2f} (the touches and a slip): "
              f"the near expiry's implied volatility {iv_front:.1%} is {kink:.2f}x the far one's {iv_back:.1%}; expected {edge:+.3f} a "
              f"share at the near expiry after costs; at most ${_held_at(price, limit) * 100 * quantity:.0f} can be lost. Out at "
              f"+{p['profit_target']:.0%}, -{p['stop_loss']:.0%}, or before the near expiry.")
    notes.append(f"{symbol}: buying the {strike:g} {right} calendar for {limit:.2f}, kink {kink:.2f}x, edge {edge:+.3f}")
    return _intent("calendar", rows, "open", quantity, limit, reason)
...
def decide(ctx):
    ...
    if ny is None:
        return {"intents": [], "cancels": [], "thought": "Term calendar: no clock.", "memory": {}}
    ...
    if not _open_day(ny.date()) or not p["entry_start"] <= clock < p["entry_end"]:
        return {"intents": intents[:8], "cancels": cancels[:20], "memory": memory,
                "thought": ("Term calendar. No entries now. " + "; ".join(notes))[:1500]}
    ...
    for symbol in NEEDS["symbols"]:
        ...
        if intent is not None:
            ...
            slots -= 1
    return {"intents": intents[:8], "cancels": cancels[:20], "thought": ("Term calendar. " + "; ".join(notes) + ".")[:1500], "memory": memory}
