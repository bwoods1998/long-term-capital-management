...
# 3-20; vol_cushion 0.8-2; min_edge 0-0.2; risk_usd 20-75; requote_minutes 5-60; slip 0-0.2 ($ a contract a leg the entry's limit pays over the touch, and a profit target's under it); max_hold_days 1-3;
# max_entries_day 1-4.
...
NEEDS = {
    ...
    "parameter_rules": {
        "bounds": {"width": [1, 5], "dte_min": [0, 3], "dte_max": [1, 7], "entry_delta": [0.1, 0.45], "profit_target": [0.2, 0.9],
                   ...
                   "min_edge": [0.0, 0.2], "risk_usd": [20, 75], "requote_minutes": [5, 60], "slip": [0.0, 0.2], "max_hold_days": [1, 3],
                   "max_entries_day": [1, 4]},
...
def _entry(ctx, p, ny, tree, symbol, notes):
    ...
    for expiry in _expiries(tree, symbol, ny.date(), int(p["dte_min"]), int(p["dte_max"])):
        ...
        for strike, row in rows.items():
            ...
            edge = _edge(price, legs, spot, real * p["vol_cushion"], p["slip"])
        ...
        reason = (f"Selling the {symbol} {expiry} {strikes} put vertical for {limit:.2f} (the touches and a slip): {why}; the short put's delta "
                  f"is {delta:+.2f}; expected {edge:+.3f} a share at expiry after costs against the realized "
                  f"{real:.2%} move to expiry; at most ${_held_at(price, limit) * 100 * quantity:.0f} can be lost. Out at "
