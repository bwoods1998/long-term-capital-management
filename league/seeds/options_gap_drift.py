# options-gap-drift: ride a single stock's news gap for days with a 3-11 day vertical in the gap's direction.
#
...
# WHAT IT NEEDS. Daily bars (40) and quotes of eight stocks with weekly options, the chain within 11 days, and
...
NEEDS = {"venue": "alpaca", "horizon": "day", "style": "options-gap-drift", "asset_class": "option", "structures": True,
         "symbols": ["INTC", "BAC", "HOOD", "PFE", "T", "F", "SOFI", "SNAP"], "bars": {"timeframe": "1Day", "limit": 40}, "max_days_to_expiry": 11,
         "wake_minutes": 10,
         "parameter_rules": {"bounds": {"width": [0.5, 5], "dte_min": [1, 11], "dte_max": [1, 11], "entry_delta": [0.2, 0.6], "profit_target": [0.2, 0.95],
...
PARAMS = {"structure": "debit_vertical", "width": 1.0, "dte_min": 3, "dte_max": 11, "entry_delta": 0.45, "profit_target": 0.6, "stop_loss": 0.5,
...
def decide(ctx):
    ...
    done = dict(memory.get("done") or {}) if isinstance(memory.get("done"), dict) else {}
    ...
    return {"intents": intents[:8], "cancels": cancels[:20], "thought": "Options gap drift. " + ("; ".join(notes) or "nothing to do") + ".",
            "memory": {"done": {k: v for k, v in done.items() if k in NEEDS["symbols"]}, "events": {k: v for k, v in events.items() if k in NEEDS["symbols"]}}}
