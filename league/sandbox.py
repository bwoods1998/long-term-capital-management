        ...
        with self._agent_lock(box_key):
            ...
            try:
                ...
                if digest not in self._state["tapes"].get(box, {}):
                    if packed is None:
                        raise TapeMissing(f"{box_key} holds no tape {digest[:12]}")
                    self.client.upload(box, path, packed, mode=0o644)
                    uploaded = True
                    with self._lock:
                ...
                self.client.upload(box, f"{self.kit_dir()}/{name}", json.dumps(spec).encode("utf-8"), mode=0o600)
                ...
                if summary is not None and summary.get("tape_missing"):
                    with self._lock:
                        ...
                        self._save()
                body = _batch_answer(summary, lambda: self.client.download(box, result_path), candidates,
