...
The next research proposal must identify the binding failures, the new information it will obtain, and its rejection condition. When this lesson was written, replay qualification required 20 trades, 20 blocks, the required out-of-sample coverage and positive growth, and deflated Sharpe at least 0.5. Since Sept 22, 2026 21:25 UTC it requires 10 trades, 20 blocks, the out-of-sample coverage and positive out-of-sample growth, with no deflated-Sharpe minimum; read `qualification_policy.replay` for the rules in force. The separate closed-trade minimum for promotion (`min_closed_trades`: five then, three since Sept 22) does not replace the replay minimum. A replay pass permits only the next authorized evaluation stage; it does not establish forward profitability.
