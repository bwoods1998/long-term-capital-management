...
`probe_gate`). A family whose pooled forward record -- the House's `family_forward`: active blocks and
summed log growth over every member ever born -- is at or below zero after `losing_min_blocks` active
...
def fold_demotions(entries: Iterable[Any], holds: dict[str, Any], states: dict[str, str],
    ...
    for entry in entries:
        last = entry.seq
        p = entry.payload
        if entry.kind == "family.record":
            if p.get("family") and p.get("venue") and p.get("state") in families.STATES:
                states[families.key_of(p["family"], p["venue"])] = p["state"]
            continue
        if p.get("decision") != "demote" or p.get("unfunded") or int(p.get("from_rung") or 0) != 2 or int(p.get("to_rung") or 0) > 1:
            continue
        known = family_of(entry.agent)
...
def audit_standing(house: Any, agent: Any) -> str:
    ...
    written against took the agent-level swing, up to 60% of the venue, on its family's approval). A
    pause or resume of its entries adopts nothing (`adopted_strategy`); an in-place edit of its PARAMS
    does."""
...
def evidence(house: Any, agent: Any, rung: int | None = None) -> Evidence:
    ...
    haircut = _paper_haircut(house, agent.id, paper_name, weights.get("alpaca_paper_haircut_bps", 0)) \
        if paper_name in HAIRCUT_BOOKS else 0.0
...
class Allocator:
    ...
    def swing_allowed(self, agent: Any) -> bool:
        ...
        if self.is_structure(agent) and not spreads_real():
            return False
    ...
    def spread_line(self, family: str, venue: str) -> dict[str, Any]:
        ...
        closed = int(mine.get("practice_closed") or 0)
        w = math.exp(max(min(float(mine.get("practice_log") or 0.0), 50.0), -50.0))
        out: dict[str, Any] = {"family": family, "meets": False, "route": None, "practice_closed": closed, "w_paper": round(w, 6),
                               "replay": None}
        if rule is None:
            ...
            if closed >= line["min_practice_closed"] and w >= line["min_w_paper"]:
                ...
                replay = self._spread_replay(family, venue, line)
                if replay is not None:
                    out.update(meets=True, route="replay", replay=replay)
            if out["route"] == "practice":
                out["why"] = (f"its family {family} has {closed} closed practice structures at W_paper {w:.4f}, at or above "
                              f"{line['min_practice_closed']} and {line['min_w_paper']:g} (allocator.spread_probe_line)")
            elif out["route"] == "replay":
                r = out["replay"]
                out["why"] = (f"its family {family} has {closed} closed practice structure(s) and a passed replay on the options "
                              f"history ({r['agent']}, {r['trades']} structures, out of sample {r['oos']:+.6f} a block; "
                              "allocator.spread_probe_line)")
            else:
                out["why"] = (f"its family {family} has {closed} closed practice structures at W_paper {w:.4f}: the line asks "
                              f"{line['min_practice_closed']} at W_paper {line['min_w_paper']:g} or more, or "
                              f"{line['replay_min_practice_closed']} and a passed replay with {line['replay_min_structures']} "
                              "structures and positive out-of-sample growth (allocator.spread_probe_line)")
        self._spread_lines[key] = out
        return out

    def _spread_replay(self, family: str, venue: str, line: Mapping[str, Any]) -> dict[str, Any] | None:
        """The best PASSED House replay (`eval.trial`) of a program the structure family ran -- a member's trial written while
        it was in the family (C8's stretches), whose `code_sha256` is a code a member ran in the family (`agent.born`,
        `agent.strategy`: a research candidate of another mechanism is not the family's) -- with at least
        `replay_min_structures` structures (a structure agent's replay counts a structure as one trade) and out-of-sample
        growth above `replay_min_oos_growth`: {"agent", "seq", "at", "trades", "oos", "code"}, or None. A structure agent's
        House replay is on the options history (`House.tape_for`); a House without one never replays it at all."""
        tape, through = self._tape, self._through
        registry = self.house.registry
        with (getattr(registry, "_lock", None) or contextlib.nullcontext()):
            everyone = [a for a in list(registry.agents.values()) if a.venue == venue]
        ...
        for agent in everyone:
            ...
            if (where is None and agent.family != family) or where == [] or not self.is_structure(agent):
                continue
            ran = {str(e.payload["code_sha256"]) for e in self.house.ledger.iter(kinds=("agent.born", "agent.strategy"), agent=agent.id)
            ...
            for entry in self.house.ledger.iter(kinds="eval.trial", agent=agent.id):
                p = entry.payload
                if not p.get("passed") or not families.within(where, entry.seq) or str(p.get("code_sha256") or "") not in ran:
                    continue
                try:
    ...
    def _drain_structure(self, agent: Any, ev: Evidence, summary: dict[str, Any]) -> None:
        """O1 off (or the rows out of their bounds) while a structure agent is on real money: it goes back to practice by the
        demotion path once that path sells nothing (`_back_when_flat`), and is lent nothing more meanwhile (`_size`). The
        House sends none of its structure OPENS to a real book meanwhile; its closes go."""
    ...
    def forward(self, family: str) -> tuple[int, float]:
        ...
        the House's own definition: every agent ever born into the family, living or dead)."""
    ...
    def gate_status(self, agent: Any, gate: Mapping[str, Any]) -> tuple[str, str]:
        ...
        only when a block closes (a `progress` row is written only when the reason changes)."""
        if gate["gate"] == "losing":
    ...
    def _unflat(self, agent: Any) -> str | None:
        ...
        def sold(holding: Any) -> bool:
            if holding.instrument.asset_class == "event":
                return False
            try:
                return dust is None or dust(book, holding.instrument, holding.quantity) is None
    ...
    def rebalance(self) -> dict[str, Any]:
        ...
        with house._lifecycle_lock:
            ...
            for agent in living:
                ...
                elif self.is_structure(agent) and not spreads_real():
            ...
            self._refold()  # this pass's demotions hold their families before any newcomer is seated
            ...
            for agent in living:
                ...
                if self.is_structure(agent) and (ev.rung == 1 or not spreads_real()):
    ...
    def _move_down(self, agent: Any, ev: Evidence, band: str, why: str, summary: dict[str, Any],
        ...
        numbers = {**self._numbers(ev, band_from, band, None, why), **dict(extra or {})}
    ...
    def _bunt(self, agent: Any, ev: Evidence, why: str, p: Mapping[str, Any], summary: dict[str, Any], displaced_at: set[str],
        ...
        gate = self.probe_gate(agent)
        if gate is not None:
            # R5 (Sept 24, 2026; `allocator.family_probe`): no probe from a losing family, nor from one a probe of which went
            # back to practice until the family's record since then has turned. Before any audit or displacement.
            stage, reason = self.gate_status(agent, gate)
            detail = {"family_forward": {"blocks": gate["blocks"], "growth": round(gate["growth"], 6)}}
            if gate["gate"] == "held":
                detail["held_since"] = gate["at"]
            house._promotion_status(agent, _verdict(agent.id, 1, why, ev), stage, reason, **detail)
            return
        ...
        if self.headroom(venue) < stake:
            ...
            # R5: a probe newcomer never displaces a probe of its own family, whose demotion would hold the family -- and
            # the newcomer with it -- the moment it was made.
            rule = families.probe_rule()
            own = agent.family if newcomer == "probe" and rule is not None and rule["hold"] else None
            weakest = self._weakest_bunt(venue, ev.e, displaced_at, tier=newcomer, state=self.family_state(agent), family=own)
            if weakest is None:
            ...
            self._refold()  # a displaced probe holds its family for the rest of the pass too (R5)
            if self.headroom(venue) < stake:
                return
        ...
        if real is None or not real.account(agent.id).funded:
            # The stake did not land: straight back, in the same pass, rather than hold an unfunded seat. `unfunded`: no real
            # dollar was lent, so it holds no family (R5, `fold_demotions`).
            house.evaluator.demote(agent.id, f"the {tier}'s stake could not be lent; back to paper",
                                   {**self._numbers(ev, tier, "paper", None, why), "unfunded": True})
            house.seat(agent)
    ...
    def _weakest_bunt(self, venue: str, e: float, displaced_at: set[str], *, tier: str = "bunt",
                      state: str | None = None, family: str | None = None) -> tuple[Any, Evidence] | None:
        ...
        undo a family swing at every full pass); a swinging family's newcomer displaces any weaker one. Nor is an
        agent of `family` displaced (R5, Sept 24, 2026): a probe newcomer's own family, which the displaced probe's
        demotion would hold at once (`allocator.family_probe`), so the seat it freed could not be the newcomer's."""
        ...
        for agent_id, ev in getattr(self, "_evidence", {}).items():
            ...
            if family is not None and agent.family == family:
                continue  # nor a probe of the newcomer's own family: its demotion would hold them both (R5)
            account = book.account(agent_id)
    ...
    def _size(self, agent: Any, ev: Evidence, band: str, p: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        if delta > 0:
            if self.is_structure(agent) and not spreads_real():
                return None  # O1 off (Sept 25, 2026): a structure agent on real money is lent nothing more; free cash still comes back
