...
`ShardFunder` is the House's mover. Once an hour, and at once after a refusal, OFF the tick (the
House's "ops" lane, never under the lifecycle lock), a pass:
...
counted from the ledger so a restart does not reset it; a shard is never drawn below the stakes
of the desks that trade there (shard 0 also carries every stake that cannot be attributed);
nothing moves while the kill switch is engaged, the grant is inactive, the House is paused or the
real Kalshi book is frozen. A failed move is a warning alert and backs that shard off for
`BACKOFF_SECONDS`.
...
class ShardFunder:
    ...
    def stakes_by_shard(self, book: Any) -> dict[int, Decimal]:
        """What each shard must keep for the desks that trade there: the real book's stakes,
        attributed by the shards of each agent's NEEDS series. A stake whose shard is not known,
        or an agent no longer in the registry, counts on the home shard (the conservative side:
        it is where deposits land and most desks trade). An agent whose series span several
        shards counts on each of them."""
        ...
        for agent_id in book.agents():
            ...
            shards: set[int] = set()
            unknown = agent is None
            for series in (self._series_of_agent(agent) if agent is not None else []):
                shard = self.shard_of(series)
                if shard is None:
                    unknown = True
                else:
                    shards.add(shard)
            if unknown or not shards:
                shards.add(HOME_SHARD)
            for shard in shards:
                out[shard] = out.get(shard, ZERO) + staked
        return out
    ...
    def run(self) -> dict[str, Any]:
        """One funding pass. Runs on the ops lane; every venue call is here. Never raises."""
        now = self.clock()
        with self._lock:
            requested = {int(s) for s in (self.state.get("requested") or [])}
        ...
        try:
            ...
            stakes = self.stakes_by_shard(book)
            ...
            for shard in sorted(wanted):
                ...
                if blocked:
                    ...
                    self._tell(f"blocked:{shard}", "warning", f"kalshi shard {shard} holds ${cash:.2f}, under the ${FLOOR_USD:.0f} floor, "
                ...
                for donor_cash, donor in sorted(((b, s) for s, b in balances.items() if s != shard), reverse=True):
                    keep = max(KEEP_USD if donor == HOME_SHARD else FLOOR_USD, stakes.get(donor, ZERO))
                    amount = min(TOP_UP_USD, donor_cash - keep, MAX_MOVE_USD, MAX_DAY_USD - day_total)
                    if amount < MIN_MOVE_USD:
                        continue
                    outcome = self._move(broker, donor, shard, amount, balances, now)
                ...
                if not moved and not any(m["destination"] == shard for m in report["moves"]):
                    ...
                    self._tell(f"short:{shard}", "warning", f"kalshi shard {shard} holds ${cash:.2f}, under the ${FLOOR_USD:.0f} floor, and {why}; "
        ...
        except Exception as exc:  # noqa: BLE001 - the next pass is an hour away, or sooner on a refusal
            ...
            with self._lock:
                ...
                self.state["last_check_epoch"] = now
                self._save()
        return report

    def _move(self, broker: Any, donor: int, shard: int, amount: Decimal, balances: dict[int, Decimal], now: float) -> dict[str, Any]:
        """One transfer, recorded whatever happens. The balances are read again after it, because
        a cross-shard move runs in up to three steps that are not undone on failure."""
        amount = amount.quantize(Decimal("0.01"))
        before = dict(balances)
        row: dict[str, Any] = {"ok": False, "usd": f"{amount:.2f}", "source": donor, "destination": shard,
        ...
        if row["ok"]:
            ...
            text = (f"kalshi collateral: moving ${amount:.2f} from shard {donor} to shard {shard} FAILED ({row['error']}); "
