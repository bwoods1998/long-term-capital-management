"""Alpaca: US equities and options, paper and live, over the same code path.
...
def instrument_for(row: dict[str, Any], *, venue: str = VENUE) -> Instrument:
    ...
    if asset_class == "crypto" or "/" in symbol:
        raise ValueError("retired asset class in Alpaca response")
...
def real_credit_types() -> "tuple[str, ...]":
    ...
    try:
        ...
        value = (CONSTITUTION.get("options_money") or {}).get("real_types")
...
class AlpacaBroker:
    ...
    def quote(self, instrument: Instrument) -> Quote:
        """Latest quote. Equities come from the selected stock feed."""
