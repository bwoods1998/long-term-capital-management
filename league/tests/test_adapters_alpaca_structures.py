...
from league.adapters import AlpacaCredentials, PURPOSE_HEADER
from league.adapters.alpaca import DATA_BASE, MLEG_TYPES, PAPER_BASE, AlpacaBroker, is_structure, mleg_body, mleg_limit
from league.broker import Instrument, OrderIntent, RejectedOrder, UnknownOutcome
...
class TheAccountsType(unittest.TestCase):
    ...
    def client(self, venue=VENUE, **account):
        from league.adapters.alpaca import LIVE_BASE
    ...
    def test_the_first_read_is_made_on_a_host_just_booted(self):
        ...
        from league.adapters.alpaca import ACCOUNT_RETRY_SECONDS

        for uptime in (0.0, 5.0, ACCOUNT_RETRY_SECONDS - 1):
            with self.subTest(uptime=uptime), mock.patch("league.adapters.alpaca.time.monotonic", return_value=uptime):
        ...
        with mock.patch("league.adapters.alpaca.time.monotonic", return_value=3.0) as clock:
    ...
    def test_the_real_account_opens_a_credit_type_only_if_the_owner_admitted_it(self):
        ...
        for admitted, opens in (("iron_condor, credit_vertical", ("credit_vertical", "iron_condor")), (["iron_butterfly"], ("iron_butterfly",)),
                                ("off", ()), (None, ()), (7, ())):
            with self.subTest(admitted=admitted), mock.patch.dict(CONSTITUTION, {"options_money": {**CONSTITUTION["options_money"], "real_types": admitted}}):
