...
class ProviderCase(unittest.TestCase):
    def setUp(self):
        ...
        self.slept = []
        self.log = Ledger(self.root / "events.sqlite", clock=lambda: self.time[0])
...
class RateCardTests(unittest.TestCase):
    def test_every_profile_carries_the_september_2026_list_price(self):
        expected = {
            ...
            # The options swarm's profiles, verified on the published card Sept 26, 2026.
...
class StaleReservationTests(ProviderCase):
    ...
    def test_the_stale_sweep_reads_the_open_requests_by_index_in_the_table_s_order(self):
        ...
        from league.provider import STALE_REQUESTS_SQL
...
class DispatchTests(ProviderCase):
    ...
    def test_provider_request_event_is_private_and_carries_the_cost(self):
        ...
        provider = self.provider(transport)
        result = self.respond(provider, profile="flash_asap")
        events = self.log.read(kinds="provider.request")
        ...
        self.assertNotIn("_response_id", __import__("league.ledger", fromlist=["public_view"]).public_view(event.payload))
        provider.close()
...
class RateCardDriftTests(ProviderCase):
    ...
    LIVE = {
        ...
        ("DeepSeek V4.1 Flash", "Default (ASAP)"): ("0.15", "0.006", "0.60"),
        ("DeepSeek V4.1 Flash", "Balanced"): ("0.12", "0.005", "0.48"),
        ("DeepSeek V4.1 Flash", "Flex"): ("0.08", "0.004", "0.30"),
        ...
        ("Kimi K3", "Balanced"): ("2.00", "0.20", "10.00"),
        ("Kimi K3", "Flex"): ("1.25", "0.15", "6.25"),
    ...
    def alerts(self):
        return [e.payload for e in self.log.read(kinds="ops.alert", limit=100)]
...
class SailBurnTests(unittest.TestCase):
    def test_the_summarys_own_spend_becomes_a_per_day_burn(self):
        from league.provider import _range_days
