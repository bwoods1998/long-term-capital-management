...
from league.broker import Instrument, OrderIntent, Position, Quote
from types import SimpleNamespace
from league.risk import Breaker, RiskContext, RiskEngine, circuit_breakers
SAMPLE = {
    "venues": ["alpaca"],
    "instruments": {"asset_classes": ["equity", "option"], "allow": [], "deny": ["GME"], "min_price": "5", "min_adv_usd": "5000000", "allow_short": False},
    "limits": {"max_position_pct": "0.25", "max_gross_pct": "1", "max_order_notional_pct": "0.25", "max_daily_loss_pct": "0.10", "max_orders_per_day": 20, "max_limit_deviation_pct": "0.10"},
    "capital": {"mode": "shadow"},
}
...
def manifest(**overrides):
    ...
    rules = data["instruments"]
    instruments = SimpleNamespace(**{k: Decimal(v) if k.startswith("min_") else v for k, v in rules.items()})
    instruments.permits_symbol = lambda symbol: symbol not in rules["deny"] and (not rules["allow"] or symbol in rules["allow"])
    return SimpleNamespace(id="earnings-01", venues=data["venues"], instruments=instruments,
                           limits=SimpleNamespace(**{k: Decimal(v) for k, v in data["limits"].items()}),
                           live=data["capital"]["mode"] == "live")
...
class RiskEngineTests(unittest.TestCase):
    ...
    def test_mandate_rules(self):
        option = Instrument("option", "SPY", "alpaca", expiry="2026-10-16", strike="450", right="call")
        only_stock = manifest(instruments={**SAMPLE["instruments"], "asset_classes": ["equity"]})
        decision = self.engine.check(intent(instrument=option, quantity="1"), context(manifest=only_stock))
        ...
        other_venue = Instrument("equity", "AAPL", "unsupported")
