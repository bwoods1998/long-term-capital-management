...
#: The fourteen founding programs of Sept 19, 2026. Rows appended later (the weather-ensemble founders of
#: Sept 25) are checked here by the rules every seed keeps, and in their own test modules for the rest.
ORIGINAL = SEEDS[:14]
...
class RegistryTests(unittest.TestCase):
    def test_fourteen_seeds_with_unique_names_and_files(self):
        self.assertGreaterEqual(len(SEEDS), 14)  # the twelve of the overnight build, and two for listed options (Sept 19, 2026)
        self.assertEqual(len({row["name"] for row in SEEDS}), len(SEEDS))
        self.assertEqual(len({row["file"] for row in ORIGINAL}), 14)
        for row in SEEDS:
            self.assertEqual(set(row), {"name", "family", "file", "why"})
            self.assertGreater(len(row["why"]), 40)

    def test_families_are_the_agreed_ones(self):
        families = {row["name"]: row["family"] for row in ORIGINAL}
    ...
    def test_every_seed_is_short_and_explains_itself_first(self):
        original = {row["name"] for row in ORIGINAL}
        for row in all_seeds():
            with self.subTest(row["name"]):
    ...
    def test_needs_and_params_are_valid(self):
        ...
        for row in all_seeds()[:len(ORIGINAL)]:
            with self.subTest(row["name"]):
    ...
    def test_the_niches_are_all_different(self):
        niches = [tuple(runner.needs_of(r["code"])["needs"][k] for k in ("venue", "horizon", "style")) for r in all_seeds()[:len(ORIGINAL)]]
