# Woods Capital runtime

`woodscapital` is the second-generation runtime of this repository: a roster of autonomous
...
3. `portfolio_runtime` moves to `docs/history/` with its run records; its market calendar,
   Yahoo adapter and evidence capture are ported into `woodscapital/data/`.
4. Repository, package, site section and docs take the Woods Capital name in one commit.
