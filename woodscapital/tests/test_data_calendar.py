...
from woodscapital.broker import Instrument, Quote
from woodscapital.data import (
    CompositeMarketData,
    DataError,
    HttpTransport,
    TransportError,
...
from woodscapital.tests.fakes import Clock
...
class _FakeSource:
    ...
    def quote(self, instrument):
        self.seen.append(("quote", instrument.key))
        return Quote(instrument, None, None, Decimal("1"), "t", self.source)

    def bars(self, instrument, interval="1d", limit=30):
        self.seen.append(("bars", interval, limit))
        return []

    def adv_usd(self, instrument):
        self.seen.append(("adv", instrument.key))
        return Decimal("42")


class CompositeTests(unittest.TestCase):
    def setUp(self):
        self.equity = _FakeSource("yahoo:delayed")
        self.crypto = _FakeSource("coinbase:advanced")
        self.event = _FakeSource("kalshi")
        self.data = CompositeMarketData(
            equity=self.equity, crypto=self.crypto, event=self.event
        )

    def test_routes_by_asset_class(self):
        self.data.quote(Instrument("equity", "AAPL", "alpaca"))
        self.data.quote(
            Instrument("option", "AAPL", "alpaca", expiry="2026-10-16", strike="200", right="call")
        )
        self.data.quote(Instrument("crypto", "BTC-USD", "coinbase"))
        self.data.quote(Instrument("event", "CPI", "kalshi", market_id="KXCPI-26SEP-T3.0"))
        self.assertEqual(len(self.equity.seen), 2)
        self.assertEqual(len(self.crypto.seen), 1)
        self.assertEqual(len(self.event.seen), 1)

    def test_bars_and_adv_follow_the_same_route(self):
        crypto = Instrument("crypto", "BTC-USD", "coinbase")
        self.data.bars(crypto, "1d", 5)
        self.assertEqual(self.crypto.seen[-1], ("bars", "1d", 5))
        self.assertEqual(self.data.adv_usd(crypto), Decimal("42"))

    def test_session_is_the_local_equity_calendar(self):
        self.assertIsNone(self.data.session("2026-11-26"))
        self.assertTrue(self.data.session("2026-11-27").early_close)

    def test_sources_are_only_built_when_used(self):
        # No equity source is configured, so the lazy import happens on first use only; asking
        # for an event quote must not construct a Yahoo client.
        data = CompositeMarketData(event=self.event)
        data.quote(Instrument("event", "CPI", "kalshi", market_id="KXCPI-26SEP-T3.0"))
        self.assertNotIn("equity", data._sources)
