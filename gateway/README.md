...
- **OpenAI credit can run out first.** The raise can pass the OpenAI account's prepaid credit, and OpenAI then refuses the call. Set `FRONTIER_MONTH_MAX_USD` to what is funded to keep the gateway's cap inside it.
