...
| `scripts/` | The owner's tools: `floor_box.py` (the House's Sailbox), `gateway_admin.py` (kill switch and status), `floor_watch.py` (the read-only watch), `gap_scoreboard.py` (the run scoreboard: the forward-first rows and the close-the-gaps metrics, read from a snapshot), `lab_box.py` (the Alpha Lab's box), and the first run's scripts. |
...
(or `--snapshot DIR`) prints the run scoreboard from a read-only snapshot of the House's stores: the
forward-first plan's seven rows with their targets, the close-the-gaps metrics, each desk's evidence
clock and every family's pooled record.
