...
class AllocationTests(CommitteeCase):
    ...
    def test_a_venue_book_shares_the_venues_equity_equally_and_reads_it_again_each_time(self):
        ...
        event = self.log.last("committee", "committee.allocation")
        self.assertIn("venue book: cash to 750.00 of alpaca's free 1500.00", event.payload["reasons"]["earnings-01"])
        grown = Committee(
            self.log, self.manifests, self.ledgers, self.provider,
            config={"floor_capital_usd": "5000", "bandit_enabled": False, "venue_book": True},
    ...
    def test_a_venue_book_keeps_its_positions_and_shares_the_free_cash(self):
        ...
        event = self.log.last("committee", "committee.allocation")
        self.assertIn("cash to 650.00 of alpaca's free 1300.00 (was -200.00; positions kept)", event.payload["reasons"]["earnings-01"])
