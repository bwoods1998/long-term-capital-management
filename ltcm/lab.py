...
KIT_API = (
    ...
    "kit.kalshi_market(ticker) -> one market (strike_type greater|less|between, floor_strike, cap_strike); kit.kalshi_markets(max_close_hours=36, pages=5) -> every open single Kalshi market settling within the window, all categories (rows can be minutes old); kit.kalshi_orderbooks(tickers) -> {ticker: {yes: [(price, count)], no: [...], yes_bid, yes_ask, no_bid, no_ask}} live books, best level first, up to 300 tickers a run; skip a market whose book is missing or has no NO bid; kit.futures(root=None) -> Coinbase futures [{symbol, root, expiry, price, volume_usd}] nearest expiry first (check the contract your market settles on: basis matters); "
    "kit.weather(city) -> {forecast..., hourly..., observation}; kit.weather_cities() -> [names] (20 Kalshi cities); "
    "kit.ensemble(city, days=3) -> {days: [{date, members: [F highs per GFS/ECMWF member], mean, sd, p10, p50, p90, n}]} and kit.bracket_probability(members, low, high) (share of members inside a temperature bracket); "
    "kit.polymarket(query='', limit=20) -> Polymarket's active markets [{question, outcomes, prices, token_ids, volume_24h, end_date}]; kit.polymarket_matches(kalshi_title) -> the same with yes_price and score, best first (check numbers and dates); kit.polymarket_book(token_id) -> {bids, asks, mid, spread}; "
    "kit.derivs(symbols) -> [{symbol, okx: {rate, next_rate, open_interest_usd}, hyperliquid: {rate, open_interest, premium}, kraken: {rate, open_interest}, dvol, funding_z}] (perps funding across venues, Deribit implied vol); kit.basis(currency) -> Deribit futures [{instrument, mark, index, basis_pct, expiry}]; "
    "kit.scoreboard(league) -> ESPN [{id, name, start, status pre|in|post, period, clock, home: {team, abbrev, score, record}, away, odds: {spread, over_under, home_ml, away_ml}}] for nfl|nba|mlb|nhl|ncaaf|ncaab|mls|epl; kit.match_game(kalshi_title, rows) -> the row naming the title's teams with side; "
    "kit.macro() -> {bls: {series: [{period, value}]}, next_fomc}. These live readers have no history: a replay refuses a strategy that calls them, so they are forward-only strategies. "
