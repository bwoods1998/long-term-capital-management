...
A new agent name or generation does not make reused history new evidence. These results failed the gates in force then: insufficient trades where applicable, nonpositive out-of-sample growth and deflated Sharpe below 0.5. Since Sept 22, 2026 21:25 UTC there is no deflated-Sharpe minimum, and nonpositive out-of-sample growth still fails. None of the four dead agents supplied forward validation.
...
4. For rosenfeld-26/27, explain how the proposed test addresses six closed trades versus the ten now required (twenty when this lesson was written) **and** nonpositive out-of-sample growth. A smaller loss alone solves neither gate. Do not force turnover to fill the quota.
