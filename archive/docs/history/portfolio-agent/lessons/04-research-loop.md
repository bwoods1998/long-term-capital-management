...
Now open the registered source notes (`experiments/data/research/source-notes.json`) and the
...
Use the checked financial packet (`experiments/data/thesis/msft-ai-infrastructure.json`) and
...
Follow these functions in investigator.py (`experiments/investigator.py`):
...
Read the [Sail product experiment](../SAIL-PRODUCTS.md). The cloud recovery test
