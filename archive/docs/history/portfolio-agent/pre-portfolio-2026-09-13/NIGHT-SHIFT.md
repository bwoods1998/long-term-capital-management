...
[Public checkpoint](https://blakewoods.us/portfolio/) · Latest measured status (`experiments/public/overnight-research.json`) · Controller (`experiments/overnight.py`)
...
The website’s cash-flow explorer was reviewed separately against source tables. Its figures are not an automatic publication of these drafts. Typed run data (`experiments/public/agent-state.json`) · Detailed product measurements (`experiments/public/overnight-sail-metrics.json`)
