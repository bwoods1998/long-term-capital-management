...
| | `allocator.longshot_floor_real` (read by `book.py`) | absent (the plan: `"0.30"`) | X0, Sept 24, 2026: a REAL Kalshi entry priced under it is refused by the risk engine's longshot rule; the floor is the larger of it and the book's `min_event_price` (0.15, which practice books keep). 20-cent ETH strikes lost twice on real money on Sept 23. Absent: 0.15 everywhere |
| | `allocator.real_entry_liquidity` (read by `book.py`) | absent (the plan: `maker_unless_family_taker_positive`) | X0: a REAL Kalshi entry must be a post-only limit unless the agent's family's pooled taker record is positive (`Allocator.family_taker`, wired to each real book as `family_taker`); an unmeasured record refuses it, and the refusal names the family, its taker settlements and bound. The taker mechanisms were the loss engine of the nine promotions of Sept 23. Absent: any order type |
| | `allocator.max_event_share` (read by `book.py`) | absent (the plan: `"0.25"`) | X0: on a REAL Kalshi book an agent's holdings at cost, working buys and the new order on one event (a ticker's first two `-` segments, Kalshi's own event: every strike of one game or one city's day, every player prop of one game; a two-segment ticker is its own event) are at most this share of its equity on the book; the refusal says "one event may hold at most 25% of the stake". Absent: no cap. `Book.risk_lines(agent)["entry_rules"]` lists the three a real book enforces |
