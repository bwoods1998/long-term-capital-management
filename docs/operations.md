...
| | `allocator.max_event_share` (read by `book.py`) | absent (the plan: `"0.25"`) | X0: on a REAL Kalshi book an agent's holdings at cost, working buys and the new order on one event (a ticker less its last `-` segment: every strike of one game or one city's day) are at most this share of its equity on the book; the refusal says "one event may hold at most 25% of the stake". Absent: no cap. `Book.risk_lines(agent)["entry_rules"]` lists the three a real book enforces |
