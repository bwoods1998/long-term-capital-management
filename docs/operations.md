...
- **The Alpha Lab** (Sept 23, 2026, `league/lab.py`). It runs only where `config.json` `lab` names a
  ...
  - **The search for capacity** (`foundry-2026-09-25.1`, S1 and S2 of the forward-first run, Sept
    25, 2026). Every card's `hypothesis.card` row carries `capacity` (`basis: "stated"`: Merton's
    ...
    replay) on its evaluation, and its birth's `route.decision` `evidence.written_for`. The dials
    stay inside `game.json` `hypotheses_bounds`. To loosen or tighten the floor, move
    `min_capacity_usd` inside its bounds (2-20).
...
| | `research.gate.practice_skip_triggers` | `book.fill`, `book.settle` | Y, from the Jev run's J2 (Sept 25, 2026; `league/research_gate.py` rule 13): a practice agent's own fills and settlements do not wake its research; anything else found (a forward block, a lesson, a note, a verdict, a refusal) still does, and an agent on real money keeps every trigger. Chosen on Sept 22-23 (kinds with 20+ practice sessions at over 3x the day's $0.138 a replay pass: fills $0.62, settlements $0.44); held out Sept 24 00Z-Sept 25 06Z it skips 13% of the dollars of the sessions F2 still runs for 2% of their replay passes. A skip reads `trigger_skip:<kind>`; the passed fills are consumed. The kinds are taken out before the pause's filter (rule 10), so a paused practice agent's fill never spends its once-a-UTC-day trade: it is woken by news of its program, its own active forward block once a day, and the sample. `[]` restores F2. Bounds: `research_bounds.gate` lists the kinds allowed |
| | `research.gate.jev_relevance` | off | Rule 4's switch (Sept 25, 2026; `league/research_gate.py`): on, the gate asks Jev whether an out-of-desk note or lesson matters to an agent it would skip, and a yes wakes the session (`jev_relevant_note`). Measured by the Jev run over Sept 22 00Z-Sept 25 14:35Z: 169 sessions woken ($5.24), o1 8.3% and o2 4.7% -- the rate of the gate's random 10% sample of would-be skips (6.6%, 4.5% over 1,481) and a fifth of the free triggers' (about 23%): $0.66 a replay pass against $0.10-0.19 (about 30 more a day since the Jev run's 11:52Z deploy lifted its call cap). Off, Jev is not asked; a note in the agent's own desk still wakes it as a free trigger. Replayed on the T0 day it removes 7 sessions ($0.12). Bounds: true or false |
| | `economy.lane_throttle` | 3 | Y1 (Sept 25, 2026; `league/yield_ledger.py` `plan_throttle`): at each hourly yield row a lane whose dollars per positive forward block over the last day exceed 3x the best lane's (the cheapest with 10+ positive blocks; the lab's own calls price its blocks), or that bought none after spending $1, is halved until a row finds it under the line: research's cadence for practice agents (twice the interval, never past a day since its last session; real money keeps its pace; Luna and Sail alike), the consultant's price, each scheduled Merton role's schedule, the engineer's pace (except a job about real money). Audits never; the lab and the foundry are priced, not halved. Each change is an `ops.budget` row `what: "lane throttle"` with `lane`, `throttled`, `usd_per_positive_block`, `best`, `ratio`, `why` and `how`; the yield row carries `throttle` every hour. Replayed on the T0 day: the lab best at $0.038, research 7.5x, the engineer 4.7x, the consultant 13.2x (F4's reading), the architect none bought: all four halved. Remove the key to switch it off: the next hourly row puts every lane still halved back (a "lane throttle" row each, `throttled: false`, `why: "economy.lane_throttle is off"`). The day is the yield rows of the last 24 hours (`DAY_ROWS`, the newest 2,000 `ops.budget` rows). Bounds 2-5 |
