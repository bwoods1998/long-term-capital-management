...
| | `lab.enabled`, `lab.budget_usd_per_hour` | on (with a `config.json` `lab.box`), $1.50 | The Alpha Lab (`league/lab.py`): its OpenAI line per trailing hour, plus royalties, inside the campaign allowance and only at frontier tier `all` |
