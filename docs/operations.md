...
- **`/workspace/state/allocator-board.json`** (Sept 23, 2026), rewritten every mark pass: each
  ...
  - every row has `family_forward` (`blocks`, `growth`: the family's pooled forward record, the House's
    ...
    (no probe is seated from the family; a probe seated on it goes back to practice), or `"held since
    <time>"` (a probe of the family went back to practice then, and the family's record since has not
    turned positive over 6 active blocks);
  ...
  - each family has `swing_clock`: `real_n`, `real_since` (its first real dollar), `real_days`,
    `real_per_day` (independent real settlements a day over its real life), `needs` (`real_settlements`
    to the next entry look, `look_at`, the look's `confidence`, whether the pooled `proof` is still
    missing, the `audit` that follows a passing look) and `days_to_swing` at its own rate. It moves with
  ...
  - `allocator.json` `probe_holds`: the ledger position the probe demotions were folded to (`cursor`),
    each family's latest probe demotion (`families`: `seq`, `at`, `agent`, `why`) and the mechanism
    ledger's state per family as its `family.record` rows said it (`states`). A House that lost the file
    folds the ledger again to the same holds.
- **`promotion_status`** has a row for each living agent on paper or above that has been judged,
  ...
  - The probe gate (R5, Sept 24, 2026; `allocator.family_probe`), before any audit or displacement:
    ...
    `family_forward`; on Alpaca, while it holds a position or a working order, it keeps its seat (its own
    exits go on, nothing is sold for it), the pass summary lists it in `probes_waiting_flat`, and one info
    alert says it goes back once it holds nothing.
...
| | `allocator.family_probe` | `losing_min_blocks` 6, `reseat` `gain_since_demotion` | No probe on a losing family (R5, Sept 24, 2026; the close-the-gaps run's third money-digest change, granted by the owner). A family is LOSING when its pooled forward record -- the House's `family_forward`: active `eval.block` count and summed log growth over every member ever born, living or dead (the allocator reads the same rows from its tape) -- is at or below zero after 6 active blocks (`families.losing`, the House's breeding line, `economy.losing_family_min_blocks`; a record that nets to zero is not a loss). Then no probe is seated from it (`family_losing`), and a probe seated on it goes back to practice at the next pass by the demotion path: a Kalshi contract is held to settlement, and on Alpaca, where that path sells, the probe keeps its seat until it holds nothing that path would sell (dust is booked, not sold) and has no working order (no sale is forced). Under `reseat` `gain_since_demotion` a probe demoted from real money for any reason (hysteresis, the stay drawdown, displacement, drift, an audit's veto, this rule) holds its family: no probe from it until the family's record SINCE that demotion is positive over 6 active blocks (`family_held`; read at every pass, so a record that turned and fell back holds again). The demotion is the ledger's own `eval.verdict` row: a probe's by its `band_from`, or, for a row that names no band (drift, audit veto, tuition), by the family's state in its last `family.record` row before it; a seat whose stake never landed (`unfunded`) holds nothing. A probe newcomer never displaces a probe of its own family. A proven or swinging family's agents are bunts and are never gated. Evidence (docs/research/queries/2026-09-24/R5-family-probe.py, 15:06Z snapshot): 11 of the allocator's 21 promotions since Sept 23 went onto such families, -$8.12 on 22 closes, no stay positive; the other 10 made +$28.96 on 34 closes. At 15:06Z 9 of 14 seated probes ($139.75 of $168.94) sat on losing families: on that snapshot the first pass would return the 6 on Kalshi (hawkins-19, hilibrand-h6ca596-3, huang-h427345-4, huang-h51fdd3-6, huang-l0c6f38, huang-l5aa23e) and the 3 crypto-alts-reversion probes on Alpaca once each is flat (haghani-62 and -63 held crypto then). Absent: probes on any record, as before |
