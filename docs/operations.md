...
- **`python3 scripts/gap_scoreboard.py --snapshot DIR | --take DIR [--since ISO] [--baseline ISO]
  ...
  docstring.
- **`python3 scripts/kalshi_watch.py [--since ISO] [--until ISO] [--json]`** (Sept 25, 2026, the
  Kalshi-scale run, [its plan](goals/LTCM_KALSHI_SCALE.md)): the weekend watch of the Kalshi desks,
  read-only on the box (sqlite `mode=ro`, as `floor_watch.py`). `--since` defaults to 30 minutes ago
  and is normalized as in `floor_watch.py`. It prints real and practice orders, fills, settlements
  and P&L by family and by league (never summed; the House's `source: dust` reconciliations are not
  fills), refused entries by class, the hours of the window with a live Kalshi order (capped at 96
  hours), the `consensus-*`, `weather-ensemble-*` and `h2h-ufc` founders (seated, waiting, why),
  capacity from the allocator's board, the Kalshi shards, the agents' data requests and the ones
  answered, every feed host's last poll and error (the I2 hosts beside the base ones) and the day's
  `web_fetch` reads by kind.
- **`python3 scripts/kalshi_capacity.py [--days 7] [--until ISO] [--family NAME ...] [--json]`**
  (Sept 25, 2026, workstream K2): what each proven or positive Kalshi family could earn a day at 1x,
  2x, 4x and 8x its size. The family's orders and records are read on the box (`mode=ro`); the depth
  and prints behind each multiple come from Kalshi's public, key-free market data, rate-limited
  (`--min-interval`, default 0.3 s) and cacheable (`--cache DIR`); `--offline` reads the ledger's
  side only. It places and cancels nothing. Its docstring gives every number's definition and
  weakness. The plan's finding (Sept 25): below 8x liquidity does not bind; the edge does.
...
| | `kalshi_founders` | on (off on the canary) | K1 (Sept 25, 2026, `league/kalshi_founders.py`): the House seats `niches.json` founder rows flagged `seat_full_league` in a full league, one a births pass, in `seat_priority` order (lower first). Off: founders wait for a free seat, as before |
| `league/niches.json` | a founder row's `seat_full_league`, `seat_priority`; a desk's `yields_seats` `{floor, reason}` | none; crypto-15m floor 4 | A flagged desk gives up its weakest practice resident (cause `desk_closed`) while it has more than `floor` members, so a founder can take the seat. Kept: real money, a position or working order, research in flight, and a winner (at least `economy.losing_family_min_blocks` active blocks over 7 days and a one-sided t of at least `kalshi_founders.WINNER_T`, 1.0). A desk that gave up a seat this tick (F3 or the seat market) gives none |
| `league/feeds.py` | `STARVE_SECONDS` | 240 s | Sept 25, 2026 (#343): a recorder that gives way to a due odds board is forced through after 240 s, one a pass, so live boards cannot starve the other recorders. Feed-by-feed texts are in `league/FEEDS.md`; `league/CONTRACT.md` keeps a line per feed |
