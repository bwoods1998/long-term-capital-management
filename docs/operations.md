...
| | `lab.batch_size`, `param_children`, `llm_children`, `leap_every` | 32, 16, 6, 10 | Candidates a batch on the lab box; parameter mutants and Luna programs bred per batch; a Sol leap every tenth Luna call |
...
| | `lab.holdout_reserve`, `stats_every_minutes`, `box_usd_per_hour` | 1, 10, $0.20 | Defaults in `league/lab.py` `DEFAULTS`, which `game.json` `lab` may override: the sealed-holdout evaluations of a living line the lab never spends (they stay with the line's own forks); how often a `lab.stats` row is written; the price the lab records for its box's time |
