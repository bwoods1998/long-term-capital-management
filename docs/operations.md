...
| | `jev.enabled`, `jev.daily_usd`, `jev.daily_calls`, `jev.purpose_calls` | on, $1.50, 25,000; gate 3,000, triage 1,500, links 1,000, exposure 500, move 7,500, memory 1,500, memory_rank 2,500 | The Jev floor: gate relevance, triage, the move sensor's shadow labels and the shared memory. One daily pool aligned to the funded Jev balance (Sept 25, 2026; $0.25 and 400 before). A breaker per purpose |
| | `jev.exposure.enabled`, `jev.hypothesis_links.enabled` | off, off | Switched off Sept 26, 2026: measured Sept 22-25, exposure groups had no consumer and never found a related pair, and hypothesis links grouped, blocked or retired no card (`docs/runs/2026-09-25-jev-senses.md`) |
| | `jev.memory` (`enabled`, `interval_seconds`, `backfill_days`, `max_docs_per_run`, `daily_usd`) | on, 600 s, 14 days, 400, $0.20 | J3, the shared memory (`league/jev_memory.py`): research conclusions, post-mortems, desk lessons and notes indexed with a Jev taxonomy (never the teacher's lessons, which only the gate's `lesson_arm` hands out); a research session's first turn shows up to 5 prior results of other agents in three arms by `sha256(agent:j3) % 3` (control: none; free ranking; Jev ranking), bounded, fail-closed, a 2 s wait (`researcher.PRIOR_TIMEOUT_SECONDS`). `python -m league.jev_memory report` is its read-only per-arm result; `graveyard` answers "has anyone tried X" |
