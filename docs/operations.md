...
- **Is it a money rule?** Compare
  ...
  unit, the family swing, `swing_requires_proven_family`, `corrected_child_supersedes`) moves them to
  constitution `b7a74074…`, money `1fa87d83…` (the grant's 101 seats unchanged: the smallest real stake
...
- **`/workspace/state/allocator-board.json`** (Sept 23, 2026), rewritten every mark pass: each
  ...
  ledger as `family.record` (private) at most every five minutes when it changed: the durable record
  of every state change. The family states and the swing's audits live in `allocator.json`
...
| | `allocator.family_proven` | 10 independent settlements, practice 0.5, real 1, 80%, `lopsided_gate` true, `unit` `at_risk`, `reference_share` 0.01 | The proof (P1): a family's pooled forward record (`families.family_record`: every member ever born, living or dead; one observation per event, members of one event pooled at the largest weight; an Alpaca practice trade pays `alpaca_paper_haircut_bps`, as E does) with a one-sided 80% lower bound (Student's t on n_eff - 1) above zero, and a lopsided record (80% or more of its observations winning: favourites) also above the House's exact loss-rate gate at 80% (`stats.lopsided_growth_lcb`, as `Evaluator._judge_family` applies it; `lopsided_gate` false restores the t bound alone). Since Deploy B an event is what it made per dollar its positions put at risk (`unit` `at_risk`: ln(1 + 1% x r) / 1%, a worthless contract -1.005); `account` restores Deploy A's account growth, under which a real row on a $30 stake weighed several times a practice row on the $200 purse. On the T0 snapshot no family is proven at risk (sports-central-run-under's account-growth proof, +0.0058, came from two strikes bought on the games it won: at risk -0.1234; weather-favorites' loss-rate bound -0.2126). The board shows `family`, `family_state`, `family_bound`, `family_n`, and "probe" as its own band in rows, the summary and the moves (the site shows it as Probe) |
| | `allocator.family_swing` | 15 real settlements, 2x the bunt, doubling every 10, capacity 0.5 on 5 markets over 7 days | The family swing (C2, Deploy B, Sept 24, 2026): a family whose REAL record has 15 independent settlements and an honest lower bound above zero (the t bound, and the loss-rate bound for favourites) swings once the frontier auditor approves its first entry (`audit:family:<family>@<venue>` on the audit lane, the family packet with `allocation_context`; `audit.verdict` rows carry `family_swing`). Every member on real money is staked at the ramp: 2 x `bunt_usd` ($60 Kalshi), doubled after each 10 further WINNING independent real settlements while the bound stays above zero, capped by the FAMILY's full Kelly on that bound against the venue's capital and 60% of it, shared by its members on real money, and held where the fill rate at the next size is under half the fill rate at the size before (the board's `stake_limit` "capacity"); never under the bunt; the envelope's headroom bounds every raise. The bound at zero or below: back to bunts (probes if the pooled proof went too), free cash only; an approval on record re-enters without a new audit. A swinging member's band is "swing" on the board and to the book (its daily-loss rule). Without the key: no family swing. At T0 no family's real record qualifies (weather-favorites: 5 real events, all won; its loss-rate bound needs about 28 clean real events at its measured edge) |
