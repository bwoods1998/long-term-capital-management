...
| | `hypotheses.fast_desks`, `fast_share` | six desks, 0.5 | Up to half the foundry's calls go to the best-scored hourly, around-the-clock desk: both Alpaca crypto desks, both Kalshi crypto desks, index ETFs and megacaps. `fast_share` 0 is off |
...
| `league/constitution.py` | `ladder.paper.settled_day` | 1 finished day once 3 trades have settled | A daily agent on a Kalshi book is screened after one finished active day once three of its trades have settled on paper. Removed: two finished days, as before Sept 23, 2026 |
...
| | `max_population`, `newcomer_seconds`, `endowment_usd` | 64, 120 s, $8 | The burst's population ceiling, newcomer cadence and House endowment |
| | `merton_schedule_hours` | operator 4, designer 12, toolsmith 3, architect 2, teacher 1 | Merton's burst cadence, set by each role's measured yield |
| `league/niches.json` | `max_members` | 6 for each Kalshi crypto desk (3 until Sept 23, 2026), 8 for each Alpaca desk, 10 for sports results, 5 otherwise | How many agents a desk may hold |
