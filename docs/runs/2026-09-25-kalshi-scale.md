...
| K1c | Tennis, MMA, golf, cricket on the ESPN league map (`SPORTS_SERIES`/`SPORTS_LEAGUES`, odds where the core API has lines) + match-winner consensus founders, tennis first | queued for the next agent slot; Deploy K1 if reviewed in time, else K2 |
| K5b | The allocator reads unlocked tranches (`Allocator.grant_capital` + `unlocked_usd`), tests (a)-(c) | after Deploy B; owner deploy |
