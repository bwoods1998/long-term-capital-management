...
- **Four loop blockers found at 17:45 UTC, fixed and deployed (17:52):** (1) the committee resized capital only on its weekday (Sunday) although `resize_interval_days` is 1, and every nightly roster change wrote an allocation that would have reset any interval measured from the last allocation; the service now forces a resize on its own schedule (`last_resize_day`), so tonight's 18:00 New York slot is the first real resize since launch. (2) Strategy orders were a fixed learning size, so a losing desk broke its own position caps and was refused forever (Scholes at $33, Haghani at $83, every order refused since the morning); `Strategies.limit_fit_usd` sizes every strategy order to fit the desk's order and position caps at 90 percent. (3) Hilibrand's `spot_quotes` treated 12 cents of BTC and 11 cents of ETH as inventory, so it stopped bidding and offered dust rounded above the holding (26 refusals in 90 minutes); dust under `min_inventory_usd` ($1) is ignored and offers round down. (4) The floor's own exits counted against `max_orders_per_day`, so hilibrand-4's stop was refused every five minutes; exits are exempt, the gateway cap still binds.
- **Firm-wide push, batch one** (18:05 UTC): the floor-wide daily loss limit was 2 percent of live equity (about $19), so one bad hour froze every live desk for the day (8.8 percent at 17:56); it is 15 percent, each desk's own daily limit still binds first. Sells now resolve to the position the desk holds on that market however the contract is spelled, and a refused sell says what is held (Haghani III spent a session guessing how to close a YES). The night watch had a 300-token cap its reasoning used up: 668 of 742 verdicts incomplete, so it never woke a desk; the cap is 2,000. Every session now reads the standings (every partner by lifetime P&L with its compute share and Brier) and the floor's calls (other partners' open probabilities), plus the rules that pay: daily capital resizes, compute scaled by P&L per Sail dollar, retirement and promotion. Strategy-opened positions belong to the strategy. Sail: the four live desks, the lab and playbook rewrites run on Kimi K3; evolution keeps six variants per family (eight max); the lab runs up to eight experiments a day, three per family, with a three-day grace.
- **Trade everything, batch two** (18:10 UTC): a survey of every open Kalshi market (combos excluded; about 790 series) showed the floor's hourly BTC and ETH range buckets are not in the top fifty by volume while the daily and hourly price thresholds (KXBTCD 1.6M contracts a day) and twenty weather cities are. `hourly_ranges` now prices buckets and above/below thresholds on all fourteen Kalshi crypto series with a Coinbase reference (BTC, ETH, SOL, XRP, DOGE, HYPE, SHIB), screens on the listing and live-quotes the sixteen biggest gaps (verified locally: 1,927 markets screened in 13 s). `KalshiMarketData.parse_market` carries strike_type, floor_strike and cap_strike. The weather table had eight wrong series names (Kalshi's newer cities carry a T: KXHIGHTPHX), so Phoenix, the second-busiest weather market, never traded; the table now has Kalshi's twenty cities with stations from the market rules, and `daily_temps` trades all of them. A `min_price` dial and new dealt variants let evolution test whether cheap longshots are the weather book's losers.
- **Sandboxes ran a stale data layer** (18:14 UTC): a diagnostic run inside scholes-4 showed /lab/floor/ltcm/data/kalshi.py predated the strike fields (only weather.py was shipped per run), so every strategy read Kalshi through the image's old parser. `FLOOR_EXTRAS` now ships broker.py and the data package (core, Kalshi, Coinbase, weather), uploaded once per sandbox per change. The kit gains `kit.futures(root)` (Coinbase's 101 listed futures: perpetual-style BTC/ETH/SOL, oil, gold, equity baskets) and the Coinbase source accepts futures product ids. Kalshi's WTI and gold were NOT mapped to them: Kalshi's WTI implied about $97.50 against $102.45 on the nearest nano crude (the October contract matched at $97.57), so a naive reference would manufacture edges.
- **Coinbase derivatives** (18:17 UTC): Coinbase lists 101 futures (perpetual-style BTC, ETH and SOL to 2030, oil, gold, equity baskets), which would give the desks shorts and leverage. The gateway now signs read-only `cfm/balance_summary` and `cfm/positions` (version 93d0bf33); the account answers `{"balance_summary": null}`: derivatives are not enabled. Enabling them is the owner's step in Coinbase; nothing on the floor can do it.
- **The firm founds families** (merged 18:27 UTC): `ltcm/founding.py` gives the floor its own line-of-business decision. At 21:30 New York, after the evolution run, one Kimi K3 call reads the floor's coverage and a 6 KB digest of what Kalshi (60,000 open markets, 1,106 series in the first smoke read, combos excluded) and Coinbase (400 USD spot products) list, and proposes one new family with an edge thesis; code validates it against the lab's hard limits, the founders' tools and the live venues, writes it born shadow as a public `evolution.founded`, and the roster reloads without a restart. Founded families that fail (best judged desk under -5 percent cost-adjusted after 4 days and 20 decisions, or no judged desk after 10 days) are retired whole; the four human families are never touched. Manual: `scripts/found_family.py --packet|--dry-run|--apply`.
- **Punishment that stopped learning** (18:38 UTC): the 18:12 roster allocation applied the drawdown breach (15 percent) to every desk: the live Scholes and Haghani sleeves went to zero, which is the rule, but so did five shadow desks' notional books (Haghani III/IV, Scholes II/III/IV), so the ranges family could not trade, be scored or learn, and the zeroed live desks kept running K3 sessions whose orders were all refused. Now the breach rules apply to live desks only; a breached live desk is cut to zero and, once its real book is flat, demoted to a shadow book (`evolution.promoted` to shadow), where it trades, is scored and can earn the sleeve back; the service applies modes every tick, builds the scoring book and cancels real resting orders. Shadow desks keep their notional book; evolution's retirement is their punishment.
- **Strategies ran a third to a sixth as often as deployed** (18:46 UTC): five-minute strategies had last run 17 to 32 minutes earlier, because at most four sandbox runs a tick ran one after another at 3 to 13 seconds each. `Strategies` now dispatches due runs to a pool (`parallel_runs` 12), one run per desk at a time, each desk on its own sandbox, off the tick; per-desk sandbox time is 4 hours a day (a 13-second scan every five minutes needs about an hour).
- **Founding corrections and the favorites starter** (18:54 UTC): the first real founding was refused for naming its desk after the committee (role names are now reserved); a refused proposal now gets one correction with the reason (the model renamed it Leahy), and was refused again because a page of tonight's Kalshi sweep failed and two of twelve targets were missing from the partial listing: failed pages are retried and a partly listed proposal keeps its listed targets. The Kalshi events family (Mullins) gets its first systematic strategy, `kalshi_favorites`: post-only NO bids a cent over the best NO bid on liquid markets across every category whose YES trades at 2 to 10 cents and settle within 36 hours (the favorite-longshot bias on Kalshi, Bürgi, Deng and Whelan 2025; makers pay no fee), one per event, two per series, never above 98 cents; verified on the live board (6,840 markets, 157 in band). `kit.kalshi_markets` lists the whole board by closing time. Five dealt variants test the band, the side of the spread and the horizon.
- **The tick held for minutes** (19:15 UTC): after the restarts the tick took 4 to 10 minutes: the night watch's model calls ran inline one after another (they now complete instead of being cut off at 300 tokens), next to the Kalshi index warm-up, the lab's one-call-per-tick night (now K3 with a 16k budget) and the committee memo. Stops, marks, orders and strategy dispatch waited on them. `Service._off_tick` runs each on its own worker (`background_work` true), one at a time, and hands the result to the next tick; roster reloads and strategy installs stay on the tick.
- **Trade notices held the tick for three minutes** (19:35 UTC box time, found with the new per-step tick timing): the notifier re-read a day of fills and outcomes every tick and kept 500 handled ids trimmed alphabetically, so real venue fills fell out behind shadow ids and were re-folded each tick at five log reads apiece. It now reads from a log cursor (`notify_seq`), passes over shadow fills and outcomes before folding, and rewinds to the first unsent notice on a gateway failure.
- **The tick is back to about 30 seconds** (19:46 UTC): per-step timing showed notices 180 s, publish 22 s (strategy records reading 40,000 events each), settlements 11 s (every held market asked about every tick). After the cursor, the shared record index and 90-second settlement sweeps, a steady tick is 28 s (publish 10-15, exits 5, feeds 3); no strategy is overdue (0 of 28, from 14). Mullins' order cap 30 -> 120 (children 15 -> 120) and Haghani's 40 -> 100, so the favorites and weather strategies are not capped out by midday.
- **A six-hour clock for capital and selection** (20:55 UTC): the committee resizes live sleeves every 6 hours (`committee.resize_interval_hours`) and evolution selects and promotes every 6 hours (`evolution_interval_hours`) instead of once a night; strategy settings promote after 6 settlements checked every 30 minutes. The promotion gate judged a desk's whole life: every tripped daily-loss breaker event counted (Haghani had 25 from one afternoon, so no such desk could ever be promoted) and one reconciliation mismatch ever failed every desk on the venue forever (the morning's NO-fill bug barred all Kalshi desks). Breakers now count as days inside a 7-day window (one allowed) and reconciliation reads the venue's latest record; the day gate is 1 day with 25 decisions. A roster allocation and a scheduled resize in one tick had collided on event ids and would have failed the tick; gate events are skipped when already written and allocation ids carry a digest of their targets.
- **First promotions to real money, and what promotion was missing** (21:09 UTC): the corrected gates passed three shadow desks and evolution promoted Haghani II (+4.4%), Mullins IV (+0.4%) and Hilibrand III. Promotion had never been exercised: a promoted desk carried its shadow book's positions (28 across the three) onto real money in its ledger, where they would fail the venue reconciliation, never settle, and let exits sell real holdings, and it kept a notional shadow sleeve until the next resize. `Service._begin_live` now runs once per promotion (keyed in service state): the shadow book's resting orders are cancelled, the shadow-filled quantity closes in the ledger at its mark (real fills since the promotion stay), and the committee funds a real sleeve at once, scaled to the venue. The same pass made the first strategy promotion: live Hilibrand's spot_quotes adopted Hilibrand II's settings.
- **A race zeroed every live sleeve** (21:18 UTC): the desk ledger's fold was not thread-safe. With strategy runs, off-tick workers and sessions now reading ledgers concurrently, a pre-allocation mark was folded after a capital withdrawal in one thread, the time-weighted return index spiked about 90 percent and the next mark read as a 45 to 68 percent drawdown on Hilibrand, Mullins, Hilibrand III, Mullins IV and Haghani II. The committee's 15 percent breach rule zeroed all five and demoted Hilibrand III and Mullins IV. A fresh fold of the same log shows drawdowns of 0.2 to 12 percent. `DeskLedger` now folds, reads, marks and resets under a lock; the restart refolds every ledger; the two demoted desks are reinstated (`evolution.promoted` back to live) and promotion setup resizes every live sleeve from its base and evidence. No real order was sent on the phantom numbers: zeroed sleeves only refuse new risk.
- **Unfunded books froze their return index** (21:40 UTC): a sleeve cut to zero kept being measured against its few dollars of leftover P&L, so a $2.50 mark move read as a 36 percent drawdown on Haghani II and an 11-point loss on Mullins IV's evidence after the race; the committee zeroed Haghani II and halved Mullins IV on it. While a book holds under a tenth of its peak capital its time-weighted index no longer moves. The promotion closes themselves realized +$7.45 (Haghani II) and +$11.91 (Mullins IV) against cost.
- **The Foundry and the backtest engine are live** (23:37 UTC): both built by a workflow with an adversarial review each (backtest: 9 findings fixed, including market selection by final volume that leaked winners and a kit path to settled results; Foundry: 12 fixed, including forward records counting pre-deployment positions and forgeable result lines). The Foundry runs every 30 minutes: baselines, 10 parameter variants and 2 K3 code mutations per family (kalshi, ranges, crypto) backtested in 8 sandboxes on a 5-day window ending 72 hours back, selected on out-of-sample evidence, deployed to the worst shadow desk and fast-tracked to live on backtest CI plus a forward record. A 14-day study of 14,000 settled Kalshi markets found no reliable favorites edge at the old settings; `kalshi_favorites` now bids YES 5-10c, 24h to close, 10,000 contracts of daily volume, NO at most 96c (+3.6% per dollar [+0.9, +5.9], walk-forward mixed). The ranges taker strategy backtested at -28% on notional.
- **Concurrency audit** (Sept 16 evening): after the ledger race, five adversarial passes over the threaded floor (tick, 12 strategy workers, sessions, off-tick workers, feed threads, founding) confirmed the bugs below; each fix is a lock and its scope, a snapshot before iterating, or a check-and-reserve, and none holds a lock across a venue call or a sandbox run. Lock order, outermost first: a desk's propose lock or a venue's ingest lock, the gateway book lock, a DeskLedger lock, the EventLog lock. Regression tests: `ltcm/tests/test_concurrency.py` (26 cases, each forcing its interleaving with a barrier or a held lock).
  - Event log reads (critical: events-log-1/2, gateway-money-1, service-state-X1, books-feeds-exits-1): `get`, `read`, `last`, `count`, `streams` and `latest_seq` ran on the shared connection without the lock `append` holds, so two readers got each other's rows, rows with no seq that wedged a ledger or gateway cursor for good, and rows an append had not committed and then rolled back. Every read now executes and fetches under `EventLog._lock` (decoded outside it); ROLLBACK failures are caught as `sqlite3.Error`; the ledger, gateway and exit book cursors skip any row at or behind them.
  - Double spend and double sell (critical: gateway-money-2; high: books-feeds-exits-5): nothing marked a proposal in flight from its risk check to its recorded order, so two proposals of one desk spent the same cash and sold the same position (an exit and a strategy sold 200 of 100 held). `Gateway.propose` re-reads the desk's book and reserves the approved order's cash, count and sell quantity under a per-desk lock, releases it once the order is recorded and swept, and the short rule counts working and in-flight sells; the exit book cancels the desk's own resting offer when it stands in the way of a stop.
  - Unattributed live fills (critical: gateway-money-3): a fill swept by another thread before the submitting worker had mapped the venue's order id was written with no desk and dropped by every ledger. The order row and venue id go into the book in one step with the end of the submission, a fill the floor cannot place waits while a submission on its venue is unanswered, the sweep of a venue is serialized from attribution to cursor, and an `EventConflict` no longer escapes `propose` after the order is live.
  - Two unknown outcomes on one desk (high: gateway-money-4): the second overwrote the first in `blocked_desks` and reconcile never looked at it again. Reconcile resolves every order still unknown, and resolving one re-blocks the desk on any other.
  - Promotions and demotions mid-proposal (high: gateway-money-5, service-state-2, books-feeds-exits-3, strategies-sandbox-3; medium: service-state-3): routing read the log while risk read the in-memory manifest and the shadow ledger, so for seconds after a promotion shadow-sized orders went to the real venue, and a demoted desk with no book was blocked forever. The service's gateway now routes by its manifest, reads the mode once per proposal, and refuses a desk whose logged mode it has not applied; `_apply_capital_modes` creates the shadow book before a demotion reaches the gateway, runs `_begin_live` before a promotion does, and waits for the desk's orders in flight first; strategy orders are sized by the mode at proposal time; a missing shadow book is a refusal, not an unknown outcome.
  - Shadow fill cursor (high: gateway-money-6, books-feeds-exits-7): one time cursor moved past fills stamped earlier than ones already swept, and they were never ingested. Sweeps read 10 minutes behind the cursor (seen fills dedupe) and the taker model stamps fills with the book's clock.
  - Shadow book reads (high: books-feeds-exits-2): the order poll read a book's connection while a worker's submit wrote it, cancelling the wrong order and raising after a fill was written. Every `ShadowBook` read takes the book's lock.
  - Exit plans of resting entries (high: books-feeds-exits-4): a maker entry was flat while it rested, so its plan closed and the position it later opened had no stop. A plan stays open while its entry order is working.
  - Order book iterated during inserts (high: service-state-7, books-feeds-exits-10; medium: gateway-money-8): feed threads, status and risk checks looped over `_orders` while workers inserted, and a second failure in `health()` ended `Service.run`. The book is copied under the gateway book lock before any loop, `run()` guards its health reports, and the feeds' held-symbol set is cached for two seconds.
  - Roster snapshots (medium: gateway-money-7, service-state-4): `live_ids`, `floor_totals`, `active_manifests` and every shadow-router loop iterate a copy while the tick adds desks.
  - Exit plans iterated during inserts (medium: books-feeds-exits-11; low: gateway-money-11): `ExitBook.plans` copies the plans first.
  - Service state writes (high: service-state-1; medium: books-feeds-exits-6): the lab worker and the tick saved `service-state.json` at once through one tmp path and wiped each other's keys. Every save and the pending-rewrite read-modify-writes hold a state lock, with a per-thread tmp name.
  - Undeployed strategies resurrected (high: strategies-sandbox-1): a run finishing after its strategy was undeployed wrote the row back, enabled with no params. Run results update only a row that still exists with the same `deployed_at`, and a run whose strategy was undeployed or redeployed proposes nothing.
  - Sandbox runs collided (high: strategies-sandbox-2): concurrent runs on one desk's box uploaded the same `/lab/run/main.py` and ran each other's code. Already fixed on main by the Foundry merge (one run per desk's sandbox at a time, `SandboxManager._run_lock`); nothing added here.
  - Promotion overwrote a redeploy (medium: strategies-sandbox-4): a strategy promotion writes only onto the house row it scored.
  - Trailing-window folds (low: events-log-3): an `until` ledger stopped at the first later stamp, missing settlements appended after it; it now reads up to 24 hours past its instant.
  - Duplicate trade notices (low: books-feeds-exits-13): the notifier folds nothing past the `latest_seq` it moves its cursor to.
  - Not fixed: service-state-5 (lab night marked done while a worker runs), service-state-9 and service-state-6 / books-feeds-exits-8 (sessions list and watch wakes), books-feeds-exits-9 (prints older than a quote), strategies-sandbox-5/6/7, service-state-8/11, gateway-money-9 (stale poll snapshot; its backwards scan cursor is fixed) and gateway-money-10: medium or low, and not one- or two-line fixes.
- **Concurrency audit and the Firm Mind are live** (Sept 16 23:47 UTC): five subsystem auditors with adversarial verification found critical races that were live on the floor: event-log reads ran unlocked on the shared SQLite connection (reproduced: fake drawdowns on unlocked ledgers, the gateway missing live resting orders so desk cash and order limits undercounted, appends reported failed after storing, lookups returning another desk's unreleased intent), plus money-path reservations missing between concurrent proposals, fill ingestion racing order recording, and service state lost updates. All critical and high findings fixed with 26 forced-interleaving tests (084de44). The Firm Mind (`ltcm/mind.py`) turns every settled outcome into rules scored in code with a time split (285 adversarial proposals on noise admitted none; a real edge admitted on its first pass), feeds the validated ones to every desk prompt and to the Foundry's code generator. 1,361 tests.
- **Where the money went, and the rules that close it off** (Sept 17 00:36 UTC box time): the account stood at $907 (−$90.55 since launch). Kalshi positions read through the gateway and marked at the bid show where the real losses came from:
  - The weather desks lost on 2-degree bracket NO bets at about 50 cents: New York, Denver and Chicago on Sept 16 cost $49.
  - The weather desks bought 1-3 cent YES tails for Sept 16-17: $41 of cost, now worth about $15.
  - One discretionary Fed position, 230 NO on a December hike at 30 cents, holds $70, which is 8% of the firm.
  - The 10-contract favorites NO positions at 93-99 cents are working.
  Two firm rules now bind every desk, shadow and live, before any other limit (`event_rules`). The first refuses any event buy under 15 cents. The second caps what one market can put at risk at 15% of desk equity, and on real money also at 3.5% of the live floor. Both legs, resting buys and the new order count toward that cap. Evidence behind the first rule: makers who bought at 10 cents or less lost 0.9 to 6.7 cents a contract over 1,054 program-days, while makers who bought favorites above 90 cents made +2.15 cents [+1.62, +2.72]. The two weather desks already hold zero equity, so the committee has starved them.
- **Shadow fees were too kind** (same deploy): every real Coinbase fill on this account paid 0.5% resting or 1.2% taking. The shadow book charged 0.15% and 0.25%, and the backtest charged 0.25% and 0.6%, so crypto quoting and reversion looked profitable in shadow and Hilibrand III was promoted on it. Both now charge the real rates. Kalshi's 160 maker-fee series, including sports games, KXFED and KXCPI, and its series fee multipliers are charged too (`ltcm/data/kalshi_fees.json`). On real money the Coinbase quotes widened to 1% a side so a round trip clears fees, and hourly reversion is paused. Shadow desks keep testing both at the real fees.
- **The favorites engine had nothing to trade** (same hour): its last run listed 8,000 markets and found 2 in band. The band was YES 5-10 cents, 1-24 hours to close and 10,000 contracts of daily volume, which left about $20 at work. On the live board, YES 4-12 cents with 1-48 hours and 2,000 contracts gives 47 markets. YES 4-15 cents with 1-48 hours and 500 contracts gives 131. Live Mullins IV now bids the first setting across 20 listing pages with 6 new bids a run. Live Mullins bids YES 5-12 cents at 1,000 contracts. The shadow desks Mullins V, VIII and III test wider, taker and tighter bands. The firm rules cap every market these desks enter.
- **Foundry**: cycle 2 was running at 00:10 when the deploy landed; its first backtests take up to 15 minutes each.
- **A sold YES came back as a bought NO** (Sept 17 00:41 UTC, 2c86820). The floor's stop sold 34 YES of a Warsh market at 15 cents, and Kalshi reported the fill as 34 NO bought at 85. The ledger then held both legs while the venue, which nets them, held neither, and the exit book re-sent the stop every few minutes with `409 order_already_exists`. The gateway now puts each attributed event fill on its order's leg. `scripts/repair_leg_fills.py` appended a reversal and a corrected copy for the four exits already on the tape: Haghani's LAX 77.5 NO, two parts of Haghani II's AUS 100.5 YES, and Mullins' Warsh YES.
- **House settings snap back on restart** (01:00 UTC): the widened favorites settings from 00:30 were reset to the house settings by the next restart's strategy bootstrap. That bootstrap resets any house row not marked `promoted_at`, and live Hilibrand's 1% spread was reset the same way. Both were re-applied with `promoted_at` set, and the next favorites runs listed a wider board: Mullins 60 markets in band, Mullins IV 42, Mullins VIII 129, Mullins V 769, each placing its full 6 or 10 bids.
- **The edge hunt's verdict** (Sept 17 01:30 UTC): three research tracks, each adversarially challenged, then a ranked plan. The plan is in the session scratchpad under `edge-hunt/BUILD_PLAN.md`.
  - **Build none of the three leads.** Kalshi's crypto markets are at least as fast as Coinbase, and quotes still stale after one second lose 1.3 to 2.5 cents. Combo quoting shows +0.19 cents [−0.45, +0.77]. A standalone liquidity-reward desk would buy the longshots the firm refuses. Coinbase long-tail market making loses at the real 0.5% maker fee, both in and out of the test window.
  - **Live Coinbase quoting is paused on Hilibrand and Hilibrand III.** An offer 1% over cost pays exactly two 0.5% maker fees, and a 90-day replay of BTC, ETH and SOL loses at every spread from 0.4% to 2%. Four resting bids were cancelled at the venue, and the account is flat in crypto.
  - **The live favorites band now stops at NO 90 cents** (YES under 10 cents). Counted one position per market, maker buys at 88-90 cents lost 0.28 cents with 11.8% losses. Buys at 90-96 cents made +1.5 to +2.0 cents, but the confidence intervals still include zero, so the edge is plausible but not yet proven.
  - **The live requote interval is now 2 hours.** The gateway's $6,000 daily cap counts dollars placed, and 30-minute requotes on the wider board would have spent it by midday.
  - **Scholes' ranges strategies are retired**, with Scholes IV kept as the control. Takers lose at every lag of one second or more, and the shadow desks are down $43-90 each.
  - A build workflow is now building the plan's code items: an evidence gate that tells luck from edge on 1:13 payoffs, a floor-wide market and cluster cap across live desks, venue-side order expiry, exact Kalshi fee rounding, favorites v2 priced from the live order book, and a fee guard in the Coinbase quoting starter.
- **The weather study: no edge** (Sept 17 02:00 UTC). The study covered 8 cities over 68 settled days, with forecasts from the exact model runs published before each decision and live station observations. The market mid still beat every signal on held-out Brier score: 0.115 against 0.136 the evening before, and 0.102 against 0.178 at 15:00. The desk's `daily_temps` rule lost 3.9 cents a contract out of sample [−7.4, −0.5], and the best in-sample rule turned negative out of sample. `daily_temps` is retired on every weather desk except the Haghani IV shadow control. The study also found that Austin had been read from Camp Mabry (KATT). Kalshi settles on Austin-Bergstrom (KAUS), and the two highs differed on 42 of 68 days; fixed in 64a3b6f.
- **The build plan's code is live** (02:40 UTC, 3596144). Three builders each worked in their own worktree, and each build was then adversarially reviewed and fixed. The full suite passed with 1,514 tests.
  - **Evidence gate** (`ltcm/evidence.py`). A lopsided event strategy is promoted or sized up only after enough settlements to separate edge from luck: 43 at an average price of 93 cents, with the Wilson upper bound of its loss rate below breakeven. Size ramps from learning size at that count to full size at three times it. Mullins and Mullins IV are back at learning size until they pass. Review fixes included the one-step size jump, a favorites record that could fall onto the easier bootstrap test, undated positions missing from the cluster cap, and an unreadable live ledger that was silently skipped.
  - **Floor-wide caps across live desks.** One market is capped at 3.5% of the live floor and one cluster at 8%. A cluster is crypto, commodities, weather or a series, grouped by close hour. The ranges family can no longer be bred, founded or promoted.
  - **Venue-side expiry** (`expires_at`: Kalshi `expiration_time`, Coinbase `limit_limit_gtd`). Kalshi fees now round up to $0.0001 on the order's running total, matching the tape. The review found that an expired Kalshi order could drop off the one page of orders the floor reads and stay open for good. The gateway now looks such an order up by its venue id a minute past expiry.
  - **Favorites v2**, priced from the live order book with a cluster cap, expiry before the final window, queue keeping and a band exit. All of these are off unless params set them. Mullins VI and VIII now run the plan's v2 shadow settings; on the first run, 39 books were read and 5 of 6 bids joined the best NO bid. The live desks keep version 1 for 24 hours. `spot_quotes` now has a fee guard, so no bid rests unless the spread covers two maker fees plus a margin. The review fixed held coins under $1 that had lost their exit offer, and an exit-only desk that re-placed unchanged offers every 15 minutes.
