...
- **Announced deploys:** Deploy V (practice; no money-digest change expected; the House release and
  ...
  **The owner's wrap-up (about 01:27Z Sept 26, all four runs):** build nothing further; ship only what is built, reviewed and
...
- **18:31Z (18:00-18:31):** 43 wakes, 3 intents (krasker-29's close, refused by the band), no structure fill. V4 deployed
  18:30-18:42Z (below).
...
**Bugs found today and their state:** the public site has refused every checkpoint since 15:50:21Z (found by the forward-first run at 20:05Z): krasker-22's CCL condor published its 95-character structure code as `market_id` and the site caps it at 80, a harness incident this run caused; fixed in `league/publish.py` by the forward-first run's PR #342 (Deploy A had already run at 20:11Z, so it ships with Deploy B at 06:00Z; the page stays frozen until about 06:12Z Saturday), and a structure-aware label (`option_label` names a structure by its first leg) goes into Deploy G; the unbounded replay-tape cache (fixed, V3); structure exits refused through the
