...
def build(kind: str, *, version: str, force: bool, api: Any = None, sleep: Callable[[float], None] = time.sleep) -> dict[str, Any]:
    ...
    try:
        source = checkpoint_with_retry(api, data_box, name=f"ltcm-data-for-{kind}-{version}", ttl_seconds=30 * 86400,
    ...
    checkpoints = []
    for label in ("a", "b"):
        row = checkpoint_with_retry(api, box, name=f"ltcm-{kind}-image-{version}-{label}", ttl_seconds=YEAR_SECONDS,
                                    sleep=sleep, errors=errors)
    ...
    entry = {
        ...
        "built_at": bl.now(), "ttl_days": 365, "sealed": {"no_network": True}, "windows": list(spec["keep"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "build":
        print(json.dumps(build(args.kind, version=args.version, force=args.force), indent=1))
